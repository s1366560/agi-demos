...
DRONE_SKIPPED_STATUSES = frozenset({"skipped"})
_DEFAULT_POLL_INTERVAL_SECONDS = 5
_HTTP_TIMEOUT_SECONDS = 30
_DRONE_CLI_JSON_TEMPLATE = "{{ json . }}"
...
class DroneCliUnavailableError(DroneConfigurationError):
    """Raised when the Drone CLI cannot be started in this runtime."""
    ...
    def from_contract(cls, contract: PipelineContractSpec) -> DronePipelineConfig:
        ...
        if not server_url:
            server_url_env = (
                ...
                or _string(raw.get("server_env"))
                or _string(raw.get("server_url_env"))
                or DRONE_SERVER_ENV
            )
            server_url = _config_env(server_url_env)
            if not server_url and server_url_env != DRONE_SERVER_URL_ENV:
                server_url = _config_env(DRONE_SERVER_URL_ENV)
            if not server_url:
...
DroneCliRunner = Callable[[tuple[str, ...], Mapping[str, str]], Awaitable[dict[str, str]]]
...
class DroneCliClient:
    """Drone client implemented through the official ``drone`` CLI."""
    ...
    def __init__(
        ...
        *,
        ...
        command: str = "drone",
        timeout_seconds: int = _HTTP_TIMEOUT_SECONDS,
        runner: DroneCliRunner | None = None,
    ) -> None:
        self._server_url = server_url.rstrip("/")
        self._token = token
        self._command = command
        self._timeout_seconds = timeout_seconds
        self._runner = runner or _run_drone_cli

    async def get_repo(self, *, owner: str, repo: str) -> Mapping[str, Any]:
        try:
            raw = await self._json("repo", "info", _repo_slug(owner, repo))
        except DroneConfigurationError as exc:
            if _looks_like_missing_repo(str(exc)):
                raise DroneRepositoryNotFoundError(
                    f"Drone repo {owner}/{repo} is not enabled"
                ) from exc
            raise
        if isinstance(raw, Mapping):
            return raw
        raise DroneConfigurationError("Drone CLI repo info response was not an object")

    async def enable_repo(self, *, owner: str, repo: str) -> Mapping[str, Any]:
        _ = await self._text("repo", "enable", _repo_slug(owner, repo))
        return {"namespace": owner, "name": repo, "active": True}
    ...
    async def update_repo(
        ...
        *,
        ...
        args = ["repo", "update", _repo_slug(owner, repo)]
        if trusted is True:
            args.append("--trusted")
        _ = await self._text(*args)
        return {"namespace": owner, "name": repo, "trusted": trusted}
    ...
    async def create_build(
        ...
        *,
        ...
        args = ["build", "create", _repo_slug(owner, repo)]
        if branch:
            args.append(f"--branch={branch}")
        if commit:
            args.append(f"--commit={commit}")
        for key, value in sorted(params.items()):
            args.append(f"--param={key}={value}")
        raw = await self._json(*args)
        if isinstance(raw, Mapping):
            return raw
        raise DroneConfigurationError("Drone CLI create build response was not an object")

    async def get_build(self, *, owner: str, repo: str, build_number: int) -> Mapping[str, Any]:
        raw = await self._json("build", "info", _repo_slug(owner, repo), str(build_number))
        if isinstance(raw, Mapping):
            return raw
        raise DroneConfigurationError("Drone CLI build info response was not an object")
    ...
    async def list_builds(
        ...
        *,
        ...
        text = await self._text(
            ...
            f"--limit={max(1, per_page)}",
            "--format",
            _DRONE_CLI_JSON_TEMPLATE,
        )
    ...
    async def stop_build(self, *, owner: str, repo: str, build_number: int) -> Mapping[str, Any]:
        _ = await self._text("build", "stop", _repo_slug(owner, repo), str(build_number))
        return {"number": build_number, "status": "killed"}
    ...
    async def _json(self, *args: str) -> object:
        text = await self._text(*args, "--format", _DRONE_CLI_JSON_TEMPLATE)
        return _json_loads(text)

    async def _text(self, *args: str) -> str:
        result = await self._runner((self._command, *args), self._env())
        if result["returncode"] != "0":
            stderr = _compact_text(result["stderr"] or result["stdout"], limit=600)
            raise DroneConfigurationError(f"Drone CLI command failed: {stderr}")
        return result["stdout"]

    def _env(self) -> Mapping[str, str]:
        return {
            **os.environ,
            DRONE_SERVER_ENV: self._server_url,
            DRONE_TOKEN_ENV: self._token,
        }


class DronePipelineProvider:
    """Runs a workspace delivery contract through the Drone CLI."""
    ...
    async def run(self, contract: PipelineContractSpec) -> PipelineRunResult:
        ...
        client = self._client or DroneCliClient(server_url=config.server_url, token=config.token)
        try:
        ...
        except DroneCliUnavailableError:
            if self._client is not None:
                return _provider_failure("Drone CLI executable not found: drone")
            return await self._run_with_http_fallback(config)
        except Exception as exc:
            return _provider_failure(str(exc))

    async def _run_with_http_fallback(self, config: DronePipelineConfig) -> PipelineRunResult:
        client = HttpDroneClient(server_url=config.server_url, token=config.token)
        try:
            await _ensure_repo_enabled(client=client, config=config)
            await _ensure_docker_deploy_repo_trusted(client=client, config=config)
            running_build = await _running_build_for_commit(client=client, config=config)
            if running_build is None:
                ...
                build_number = _required_int(running_build.get("number"), "Drone build number")
            build = await self._poll_build(client=client, config=config, build_number=build_number)
            result = await _result_from_build(client=client, config=config, build=build)
            return replace(
                result,
                metadata={**dict(result.metadata or {}), "drone_client": "http_fallback"},
            )
        except Exception as exc:
            return _provider_failure(str(exc))
...
async def _run_drone_cli(args: tuple[str, ...], env: Mapping[str, str]) -> dict[str, str]:
    try:
        ...
        raise DroneCliUnavailableError("Drone CLI executable not found: drone") from exc
...
def _repo_slug(owner: str, repo: str) -> str:
    return f"{owner}/{repo}"
...
def _json_loads(value: str) -> object:
    ...
    try:
        ...
        raise DroneConfigurationError(f"Drone CLI returned invalid JSON: {exc}") from exc


def _looks_like_missing_repo(value: str) -> bool:
    normalized = value.lower()
    return "not found" in normalized or "404" in normalized or "not enabled" in normalized
...
def _internal_status(status: str) -> str:
    if status == "success":
        return "success"
    if status in DRONE_SKIPPED_STATUSES:
        return "skipped"
