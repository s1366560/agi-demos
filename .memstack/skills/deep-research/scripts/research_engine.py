        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    ...
                    raise OSError(f"Failed to save state after {max_retries} attempts: {e}") from e
...
""",
    ...
    def run_pipeline(self, query: str) -> str:
        ...
        for phase in phases:
            ...
            _result = self.execute_phase(phase)

            # Save state after each phase
            state_file = (
                self.output_dir / f"research_state_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
        ...
        report_file = (
            self.output_dir / f"research_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
