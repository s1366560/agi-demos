...
describe('TimelineEventItem - StepStart Event Handling', () => {
  it('should not render step_start when stepDescription is missing', () => {
    ...
    // Should not render anything when description is empty
    expect(container.firstChild).toBe(null);
  });

  it('should not render step_start when both stepIndex and stepDescription are undefined', () => {
    const event: TimelineEvent = {
      id: 'step-2',
      type: 'step_start',
      sequenceNumber: 1,
    ...
    // Should not render anything when data is missing
    expect(container.firstChild).toBe(null);
  });

  it('should render step_start when stepDescription exists', () => {
    const event: TimelineEvent = {
      id: 'step-3',
      type: 'step_start',
      sequenceNumber: 1,
      timestamp: Date.now(),
      stepIndex: 1,
      stepDescription: 'Search memory for patterns',
    } as any;

    const { container } = render(<TimelineEventItem event={event} />);

    // Should render the description
    expect(screen.getByText(/Search memory for patterns/)).toBeInTheDocument();
  });

  it('should render step_start without index when only description exists', () => {
    const event: TimelineEvent = {
      id: 'step-4',
      type: 'step_start',
      sequenceNumber: 1,
    ...
    // Should render the description without index
