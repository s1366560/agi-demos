...
vi.mock('../../../../components/agent/execution/TimelineNode', () => ({
  TimelineNode: ({ step, isExpanded, isCurrent, onToggle }: {
...
vi.mock('../../../../components/agent/execution/SimpleExecutionView', () => ({
  SimpleExecutionView: ({ toolExecutions, isStreaming }: {
...
vi.mock('../../../../components/agent/shared', () => ({
  MaterialIcon: ({ name, size, className }: { name: string; size?: number; className?: string }) => (
...
describe('ExecutionTimeline (Compound Components)', () => {
  describe('getDisplayMode utility function', () => {
    ...
    it('should return "direct" when no work plan and single step/tool', () => {
      const mode = getDisplayMode(null, [{ stepNumber: 1, description: 'Test', status: 'completed', thoughts: [], toolExecutions: [] }], [{ id: '1', toolName: 'test', input: {}, status: 'success', startTime: '2024-01-01T00:00:00Z' }])
      expect(mode).toBe('direct')
    })
  ...
  describe('ExecutionTimeline - Main Container', () => {
    it('should return null for direct mode (no rendering)', async () => {
      const { default: ExecutionTimeline } = await import('../../../../components/agent/execution/ExecutionTimeline')
  ...
  describe('ExecutionTimeline - Work Plan Checklist', () => {
    ...
    it('should show tool count for steps with executions', async () => {
      ...
      const stepsWithTools = mockSteps.map(s => ({
        ...s,
        toolExecutions: [{ id: 'tool-1', toolName: 'test', input: {}, status: 'success', startTime: '2024-01-01T00:00:00Z' }],
      }))
      render(
        <ExecutionTimeline
          workPlan={mockWorkPlan}
  ...
  describe('ExecutionTimeline - Failed Steps', () => {
    it('should show failed status for failed steps', async () => {
      const { default: ExecutionTimeline } = await import('../../../../components/agent/execution/ExecutionTimeline')
      const failedSteps = [
        { ...mockSteps[0], status: 'failed' as const },
        ...mockSteps.slice(1),
      ]
      render(
        <ExecutionTimeline
