...
describe('sandboxService', () => {
  ...
  describe('getSandbox', () => {
    ...
    it('should handle 404 when sandbox not found', async () => {
      const apiError = new ApiError(ApiErrorType.NOT_FOUND, 'NOT_FOUND', 'Sandbox not found', 404);
      mockHttpClient.get.mockRejectedValue(apiError);

      await expect(sandboxService.getSandbox('invalid')).rejects.toThrow('Sandbox not found');
    });
  });
  ...
  describe('stopDesktop', () => {
    ...
    it('should handle stop desktop failure', async () => {
      ...
      await expect(sandboxService.stopDesktop('sb_123')).rejects.toThrow('Failed to stop desktop');
    });
  });
  ...
  describe('startTerminal', () => {
    it('should start terminal service and return constructed WebSocket URL', async () => {
      ...
      expect(result.url).toMatch(
        /ws:\/\/localhost:\d+\/api\/v1\/terminal\/sb_123\/ws\?session_id=sess_abc123/
      );
