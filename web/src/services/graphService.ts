...
export const graphService = {
    ...
    async listEntities(params: {
        ...
        const response = await apiClient.get<{ entities: Entity[]; items: Entity[]; total: number; limit?: number; offset?: number }>(`/graph/entities/?${queryParams.toString()}`)
        return {
            items: response.entities || response.items || [],
            total: response.total,
            limit: response.limit ?? 50,
            offset: response.offset ?? 0,
            has_more: (response.offset ?? 0) + (response.entities || response.items || []).length < response.total
        }
    },
    ...
    async getEntityRelationships(
        ...
        return await apiClient.get<EntityRelationships>(`/graph/entities/${entityId}/relationships?${queryParams.toString()}`)
    },
    ...
    async listCommunities(params: {
        ...
        return await apiClient.get<{ communities: Community[]; total: number; limit?: number; offset?: number }>(`/graph/communities/?${queryParams.toString()}`)
    },

    async getCommunityMembers(communityId: string, limit = 100): Promise<CommunityMembers> {
        return await apiClient.get<CommunityMembers>(`/graph/communities/${communityId}/members?limit=${limit}`)
    },
    ...
    async searchWithFacets(params: {
        ...
        return await apiClient.post<SearchResults & { limit: number; offset: number }>('/search-enhanced/faceted', params)
    },
    ...
    async getEmbeddingStatus(projectId: string): Promise<EmbeddingStatus> {
        return await apiClient.get<EmbeddingStatus>(`/maintenance/embeddings/status?project_id=${projectId}`)
    },

    async rebuildEmbeddings(projectId: string): Promise<RebuildEmbeddingsResult> {
        return await apiClient.post<RebuildEmbeddingsResult>(`/maintenance/embeddings/rebuild?project_id=${projectId}`)
    },
