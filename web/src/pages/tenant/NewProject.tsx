    ...
    return (
        <div className="max-w-5xl mx-auto flex flex-col gap-8 pb-10 px-6">
            ...
            <div className="flex flex-col gap-1">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{t('tenant.newProject.title')}</h1>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                ...
                <div className="bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t('tenant.newProject.basicInfo')}</h2>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm h-full">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t('tenant.newProject.memoryRules')}</h2>
                        </div>

                        <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.newProject.maxEpisodes')}</label>
                                    <input
                                        type="number"
                                        min="100"
                                        max="10000"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.newProject.retentionDays')}</label>
                                    <input
                                        type="number"
                                        min="1"
                                        max="365"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.newProject.refreshInterval')}</label>
                                <input
                                    type="number"
                                    min="1"
                                    max="168"
                            ...
                            <div className="flex items-center gap-3 pt-2">
                                ...
                                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('tenant.newProject.enableAutoRefresh')}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm h-full">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t('tenant.newProject.graphConfig')}</h2>
                        </div>

                        <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.newProject.maxNodes')}</label>
                                    <input
                                        type="number"
                                        min="100"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.newProject.maxEdges')}</label>
                                    <input
                                        type="number"
                                        min="100"
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">{t('tenant.newProject.similarityThreshold')}</label>
                            ...
                            <div className="flex items-center gap-3 pt-2">
                                ...
                                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('tenant.newProject.enableCommunityDetection')}</span>
                            </div>
                        </div>
                    </div>
                </div>
