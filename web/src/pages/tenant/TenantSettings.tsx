        ...
        <div className="max-w-full mx-auto flex flex-col gap-8">
            ...
            <div className="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 md:p-8">
                ...
                <div className="flex flex-col gap-6 max-w-2xl">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.settings.general.name')}</label>
                        <input
                            type="text"
                            value={name}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t('tenant.settings.general.description')}</label>
                        <textarea
                            value={description}
                    ...
                    {message && (
                        <div className={`p-4 rounded-lg flex items-center gap-3 ${message.type === 'success' ? 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300' : 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('tenant.settings.plan.current')}</h3>
                                <p className="text-3xl font-bold text-slate-900 dark:text-white mt-2 capitalize">{tenant.plan}</p>
                            </div>
                            <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">Active</span>
                        </div>
                        <p className="text-sm text-slate-500 mb-6">{t('tenant.settings.plan.active_since', { date: new Date(tenant.created_at).toLocaleDateString() })}</p>
                        <button className="text-primary hover:text-primary-dark font-medium text-sm flex items-center gap-1">
                            {t('tenant.settings.plan.change')} <span className="material-symbols-outlined text-[16px]">arrow_forward</span>
                        </button>
                    </div>

                    <div className="flex flex-col gap-4">
                        <h3 className="text-sm font-medium text-slate-900 dark:text-white">{t('tenant.settings.plan.limits')}</h3>
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between text-sm mb-1">
                                    <span className="text-slate-600 dark:text-slate-300">{t('tenant.settings.plan.projects')}</span>
                            ...
                            <div>
                                <div className="flex justify-between text-sm mb-1">
                                    <span className="text-slate-600 dark:text-slate-300">{t('tenant.settings.plan.storage')}</span>
            ...
            <div className="bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-200 dark:border-red-900/30 p-6 md:p-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h3 className="font-medium text-red-900 dark:text-red-300">{t('tenant.settings.danger.delete_title')}</h3>
                        <p className="text-sm text-red-700 dark:text-red-400 mt-1 max-w-xl">{t('tenant.settings.danger.delete_desc')}</p>
                    </div>
