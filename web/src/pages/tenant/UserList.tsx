...
export const UserList: React.FC = () => {
    ...
    const filteredMembers = members.filter(member => {
        ...
        const matchesStatus = statusFilter === 'all' || 'active' === statusFilter.toLowerCase()
    ...
    return (
        <div className="max-w-[1400px] mx-auto w-full flex flex-col gap-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{t('tenant.users.title')}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-start justify-between">
                    <div>
                        <p className="text-sm font-medium text-slate-500 mb-1">{t('common.stats.activeUsers')}</p>
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-start justify-between">
                    <div>
                        <p className="text-sm font-medium text-slate-500 mb-1">{t('common.stats.pendingInvites')}</p>
                    ...
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead className="bg-slate-50 dark:bg-slate-800">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider" scope="col">{t('common.forms.role')}</th>
                        ...
                        <tbody className="bg-surface-light dark:bg-surface-dark divide-y divide-slate-200 dark:divide-slate-700">
                            {isLoading ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-slate-500">{t('tenant.users.loading') || 'Loading members...'}</td>
                                </tr>
                            ) : filteredMembers.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-slate-500">{t('tenant.users.noMembers')}</td>
                                </tr>
                            ) : (
                    ...
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p className="text-sm text-slate-700 dark:text-slate-400">
                                {t('tenant.users.showingResults', { start: 1, end: filteredMembers.length, total: filteredMembers.length })}
                            </p>
                        </div>
                        <div>
