...
export const Maintenance: React.FC = () => {
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 md:p-8 flex flex-col gap-8">
            {/* Header */}
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('project.maintenance.title')}</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">{t('project.maintenance.subtitle')}</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm text-center">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">{stats?.entity_count || '-'}</p>
                    <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">{t('project.maintenance.stats.entities')}</p>
                </div>
                <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm text-center">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">{stats?.episodic_count || '-'}</p>
                    <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">{t('project.maintenance.stats.episodes')}</p>
                </div>
                <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm text-center">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">{stats?.community_count || '-'}</p>
                    <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">{t('project.maintenance.stats.communities')}</p>
                </div>
                <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm text-center">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">{stats?.edge_count || '-'}</p>
                    <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">{t('project.maintenance.stats.relationships')}</p>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <div className="divide-y divide-slate-200 dark:divide-slate-800">
                    <MaintenanceOperation
                        ...
                        icon="refresh"
                        actionLabel={refreshLoading ? t('project.maintenance.ops.refresh.loading') : t('project.maintenance.ops.refresh.button')}
                    ...
                    <MaintenanceOperation
                        ...
                        icon="content_copy"
                        actionLabel={dedupProcessing ? t('project.maintenance.ops.dedup.processing') : t('project.maintenance.ops.dedup.merge')}
                    ...
                    <MaintenanceOperation
                        ...
                        actionLabel={cleanProcessing ? t('project.maintenance.ops.clean.cleaning') : t('project.maintenance.ops.clean.clean')}
                    ...
                    <MaintenanceOperation
                        ...
                        icon="group_work"
                        actionLabel={rebuildLoading ? t('project.maintenance.ops.rebuild.rebuilding') : t('project.maintenance.ops.rebuild.button')}
                        ...
                        <MaintenanceOperation
                            ...
                            description={
                                <div className="flex items-center gap-3">
                                    <span>{embeddingStatus.current_provider} ({embeddingStatus.current_dimension}维)</span>
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-200 dark:border-indigo-800 p-6">
                ...
                <div className="space-y-3">
                    <div className="flex items-start gap-3 p-3 bg-white dark:bg-surface-dark rounded-lg border border-indigo-100 dark:border-indigo-800/50 shadow-sm">
                        ...
                        <div>
                            <p className="font-medium text-slate-900 dark:text-white">{t('project.maintenance.recommendations.high_duplication.title')}</p>
                            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{t('project.maintenance.recommendations.high_duplication.desc', { count: 45 })}</p>
                        </div>
                    </div>
                </div>
            </div>
