...
export const MemoryDetail: React.FC = () => {
        ...
        const fetchMemory = async () => {
            if (projectId && memoryId) {
                ...
                try {
                    ...
                    if ((data.processing_status === 'PENDING' || data.processing_status === 'PROCESSING') && data.task_id) {
    ...
    if (!projectId || !memoryId) {
        return <div className="p-8 text-center text-slate-500">{t('project.memories.detail.invalid_params')}</div>
    }

    if (isLoading) {
        return <div className="p-8 text-center text-slate-500">{t('project.memories.detail.loading')}</div>
    }

    if (!memory) {
        return <div className="p-8 text-center text-slate-500">{t('project.memories.detail.not_found')}</div>
    }
    ...
    const handleReprocess = async () => {
        ...
        try {
            ...
            setMemory(prev => prev ? { ...prev, processing_status: 'PENDING', task_id: response.task_id } : null)
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                ...
                <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-surface-light/80 dark:bg-surface-dark/80 backdrop-blur-md flex items-center justify-between px-6 z-10 sticky top-0">
                    ...
                    <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
                        <Link to={`/project/${projectId}`} className="text-slate-500 hover:text-primary text-sm font-medium transition-colors">{t('common.project')}</Link>
                        <span className="text-slate-400 text-sm">/</span>
                        <Link to={`/project/${projectId}/memories`} className="text-slate-500 hover:text-primary text-sm font-medium transition-colors">{t('project.memories.title')}</Link>
                        <span className="text-slate-400 text-sm">/</span>
                        <div className="flex items-center gap-2">
                            <span className="text-slate-900 dark:text-white text-sm font-medium truncate max-w-[200px]">{memory.title || t('common.untitled')}</span>
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            disabled={isReprocessing || memory.processing_status === 'PROCESSING' || memory.processing_status === 'PENDING'}
                            className={`p-2 rounded-lg transition-all flex items-center gap-1 ${isReprocessing || memory.processing_status === 'PROCESSING' || memory.processing_status === 'PENDING'
                        ...
                        >
                            <span className={`material-symbols-outlined text-[20px] ${isReprocessing ? 'animate-spin' : ''}`}>
                                {isReprocessing ? 'progress_activity' : 'refresh'}
                            </span>
                        </button>
                        <button onClick={() => setEditModalOpen(true)} className="p-2 text-slate-500 hover:text-primary hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 rounded-lg transition-all" title="Edit">
                ...
                <div className="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark">
                    <div className="max-w-5xl mx-auto p-6 md:p-8 flex flex-col gap-6">
                        ...
                        <div className="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                                    ...
                                    <div className="flex gap-5 items-center">
                                        <div className="relative">
                                            ...
                                            <div className="absolute -bottom-1 -right-1 bg-white dark:bg-slate-900 rounded-full p-1 shadow-sm border border-slate-100 dark:border-slate-700">
                                                <div className={`rounded-full h-3 w-3 ${memory.status === 'DISABLED' ? 'bg-red-500' : 'bg-green-500'}`} title={memory.status === 'DISABLED' ? t('common.status.unavailable') : t('common.status.available')}></div>
                                            </div>
                                        </div>
                                        <div className="flex flex-col justify-center gap-1">
                                            <h1 className="text-slate-900 dark:text-white text-2xl md:text-3xl font-bold leading-tight tracking-tight">{memory.title || t('common.untitled')}</h1>
                                            <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base font-normal flex items-center gap-2 flex-wrap">
                                                <span>{t('project.memories.detail.type')}: <span className="text-slate-900 dark:text-slate-200 font-medium capitalize">{memory.content_type || t('common.unknown')}</span></span>
                                                <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                                                <span>{new Date(memory.created_at).toLocaleDateString()}</span>
                                            </p>
                                        </div>
                                    </div>
                                    <button onClick={() => setEditModalOpen(true)} className="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-colors shadow-sm shadow-primary/20">
                            ...
                            {(processingProgress || memory.processing_status === 'PENDING' || memory.processing_status === 'PROCESSING') && (
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-600 dark:text-slate-400">
                                                    {processingProgress?.message || t('project.memories.detail.extracting_knowledge') || 'Extracting knowledge...'}
                                                </p>
                                            </div>
                                        </div>
                                ...
                                {activeTab === 'metadata' && (
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                                            <span className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('project.memories.detail.metadata.id')}</span>
                                            <span className="font-mono break-all">{memory.id}</span>
                                        </div>
                                        <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                                            <span className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('project.memories.detail.type')}</span>
                                            <span className="capitalize">{memory.content_type}</span>
                                        </div>
                                        <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-lg col-span-2">
                                            <span className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('project.memories.detail.metadata.custom')}</span>
            ...
            <aside className="w-80 bg-surface-light dark:bg-surface-dark border-l border-slate-200 dark:border-slate-800 hidden xl:flex flex-col flex-shrink-0 z-10">
                ...
                <div className="overflow-y-auto flex-1 p-5 flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-3">
                        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">{t('project.memories.detail.sidebar.tags')}</h3>
                        <div className="flex flex-wrap gap-2">
                            ...
                            )) || <span className="text-xs text-slate-500">{t('project.memories.detail.sidebar.no_tags')}</span>}
                        </div>
                    </div>
                </div>
            </aside>
