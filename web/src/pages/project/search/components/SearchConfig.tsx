...
export const SearchConfig = memo<SearchConfigProps>(({
    ...
    const toggleRelationshipType = useCallback((rel: string) => {
        onRelationshipTypesChange(
...
const RetrievalModeSelector = memo<{ value: RetrievalMode; onChange: (mode: RetrievalMode) => void }>(({ value, onChange }) => (
...
const StrategySelector = memo<{ value: string; onChange: (value: string) => void }>(({ value, onChange }) => (
    <div className="flex flex-col gap-2">
...
const CrossEncoderSelector = memo<{ value: string; onChange: (value: string) => void }>(({ value, onChange }) => (
    <div className="flex flex-col gap-2">
...
const GraphTraversalParams = memo<GraphTraversalParamsProps>(({ maxDepth, relationshipTypes, onMaxDepthChange, onToggleRelationshipType }) => (
    <>
        <div className="flex flex-col gap-2">
...
const TemporalFilters = memo<TemporalFiltersProps>(({ timeRange, customTimeRange, onTimeRangeChange, onCustomTimeRangeChange }) => (
    <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Time Range</h3>
            <button onClick={() => { onTimeRangeChange('all'); onCustomTimeRangeChange({}) }} className="text-xs text-blue-600 hover:underline font-medium">Reset</button>
        </div>
            ...
            <div className="flex flex-col gap-3">
                <div>
                    ...
                    <input
                        ...
                        onChange={(e) => onCustomTimeRangeChange({ ...customTimeRange, since: e.target.value ? new Date(e.target.value).toISOString() : undefined })}
                ...
                <div>
                    ...
                    <input
                        ...
                        onChange={(e) => onCustomTimeRangeChange({ ...customTimeRange, until: e.target.value ? new Date(e.target.value).toISOString() : undefined })}
                        className="w-full text-xs py-2.5 px-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-1 focus:ring-blue-600 text-slate-700 dark:text-slate-200"
                    />
                </div>
            </div>
...
const FacetedFilters = memo<FacetedFiltersProps>(({ selectedEntityTypes, selectedTags, availableTags, onToggleEntityType, onToggleTag }) => (
    <>
        <div className="flex flex-col gap-3">
...
const CommunityFilters = memo<CommunityFiltersProps>(({ includeEpisodes, onIncludeEpisodesChange }) => (
    <>
        <div className="flex flex-col gap-2">
