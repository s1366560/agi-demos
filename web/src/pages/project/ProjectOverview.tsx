...
export const ProjectOverview: React.FC = () => {
    ...
    const getMemoryStatus = (memory: Memory) => {
        if (memory.status === 'DISABLED') {
            return { label: t('common.status.unavailable'), color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', dot: 'bg-red-500' }
        }
        return { label: t('common.status.available'), color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', dot: 'bg-green-500' }
    }

    const getMemoryTitle = (memory: Memory) => {
        // If title is generic (same as content_type) or empty, generate a better title
        if (!memory.title || memory.title === memory.content_type || memory.title === 'description' || memory.title === 'text') {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col gap-1">
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{t('project.overview.title')}</h2>
                <p className="text-slate-500 dark:text-slate-400">{t('project.overview.subtitle', { name: project?.name || 'Project' })}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between h-32 hover:border-primary/50 transition-colors group">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('common.stats.totalMemories')}</span>
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between h-32 hover:border-primary/50 transition-colors group">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('common.stats.storageUsed')}</span>
                    ...
                    <span className="text-xs text-slate-500 mt-1">{t('project.overview.quotaUsage', { percent: Math.round((stats.storage_used / stats.storage_limit) * 100) })}</span>
                </div>
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between h-32 hover:border-primary/50 transition-colors group">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('common.stats.activeNodes')}</span>
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between h-32 hover:border-primary/50 transition-colors group">
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('common.stats.collaborators')}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t('project.overview.activeMemories')}</h3>
                        <Link to={`/project/${projectId}/memories`} className="text-sm text-primary font-medium hover:text-primary/80">{t('common.actions.viewAll')}</Link>
                    </div>
                    <div className="bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm overflow-hidden">
                            ...
                            <table className="w-full text-left text-sm">
                                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400">{t('common.forms.status')}</th>
                                        <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400 text-right">{t('project.memories.size')}</th>
                                                ...
                                                >
                                                    <td className="px-6 py-3">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <div className="font-medium text-slate-900 dark:text-white">{getMemoryTitle(memory)}</div>
                                                                <div className="text-xs text-slate-500">
                                                                    {t('common.time.updated', { time: formatDate(memory.updated_at || memory.created_at) })}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                        ...
                        <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-surface-dark flex justify-center">
                            <button className="text-xs font-medium text-slate-500 hover:text-primary flex items-center gap-1">
                                {t('common.actions.showMore')} <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>expand_more</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-lg shadow-sm p-5">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide">{t('project.overview.projectTeam')}</h3>
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-slate-900 dark:text-white">{stats?.collaborators || 0} {t('common.stats.members')}</p>
                    ...
                    <div className="bg-primary text-white rounded-lg shadow-lg p-5 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <h3 className="text-sm font-bold uppercase tracking-wide mb-3">{t('common.stats.systemStatus')}</h3>
