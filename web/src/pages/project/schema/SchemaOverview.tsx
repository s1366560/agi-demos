...
const EntityCard = memo(({ entity, t }: EntityCardProps) => (
  <div className="group relative flex flex-col gap-4 rounded-xl border border-slate-200 dark:border-[#252d46] bg-white dark:bg-[#1c2333] p-5 hover:border-emerald-500/50 dark:hover:border-emerald-500/30 transition-all hover:shadow-lg hover:shadow-emerald-900/5">
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 dark:text-[#95a0c6] text-sm">
    ...
    <div className="flex flex-col gap-2">
      <p className="text-xs font-semibold text-slate-400 dark:text-[#56607a] uppercase tracking-wider">
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(entity.schema || {})
          ...
          .map(([key, val]: [string, any]) => (
            <span
              key={key}
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-slate-50 dark:bg-[#111521] border border-slate-200 dark:border-[#252d46] text-xs font-mono text-slate-500 dark:text-[#95a0c6]"
            >
              <span
  ...
  <div className="group relative flex flex-col gap-4 rounded-xl border border-slate-200 dark:border-[#252d46] bg-white dark:bg-[#1c2333] p-5 hover:border-blue-500/50 dark:hover:border-[#193db3]/50 transition-all hover:shadow-lg hover:shadow-blue-900/5">
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h4 className="text-slate-900 dark:text-white font-bold text-base group-hover:text-blue-600 dark:group-hover:text-[#193db3] transition-colors">
              {edge.name}
            </h4>
          ...
          <p className="text-slate-500 dark:text-[#95a0c6] text-sm font-mono">
  ...
  if (filteredMappings.length === 0) {
    return (
      <div className="flex items-center justify-center p-3 rounded-lg bg-slate-50 dark:bg-[#111521] border border-slate-200 dark:border-[#252d46] border-dashed">
        <span className="text-xs text-slate-400 dark:text-[#56607a] italic">
  ...
  return (
    <div className="flex flex-col gap-2">
      {filteredMappings.map((map) => (
        <div
          key={map.id}
          className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 dark:bg-[#111521] border border-slate-200 dark:border-[#252d46]"
        >
          ...
          <div className="flex-1 flex flex-col items-center gap-1 relative">
            ...
            {map.source === 'generated' && (
              <span className="absolute -top-2 text-[8px] uppercase tracking-wider text-purple-600 dark:text-purple-400 font-bold bg-white dark:bg-[#111521] px-1">
                {String(t('project.schema.overview.auto'))}
              </span>
            )}
          </div>
  ...
  <div className="flex flex-col gap-2">
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {Object.keys(edge.schema || {}).length === 0 && (
        <span className="text-slate-400 dark:text-[#95a0c6] text-xs italic">
          ...
          <div className="w-full">
            <div className="relative group">
              ...
              <div className="absolute inset-y-0 right-2 flex items-center">
                <button className="px-2 py-1 text-xs font-medium text-slate-500 dark:text-[#95a0c6] bg-slate-100 dark:bg-[#111521] rounded border border-slate-200 dark:border-[#252d46]">
                  CMD + K
                </button>
              </div>
            </div>
            ...
            <section className="flex flex-col gap-5">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-[#252d46]">
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-[#252d46] text-slate-500 dark:text-[#95a0c6] text-xs font-mono">
