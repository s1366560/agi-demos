...
const TEXTS = {
  ...
  relationshipsComingSoon: 'Define how this entity type connects to others. This feature is coming in the next update.',
    ...
    <div className="w-full flex-none pt-8 pb-4 px-8 border-b border-slate-200 dark:border-[#2a324a]/50 bg-white dark:bg-[#121521]">
      <div className="max-w-7xl mx-auto flex flex-col gap-4">
        <div className="flex flex-wrap justify-between items-center gap-4">
          <div>
            <h2 className="text-slate-900 dark:text-white text-3xl font-bold tracking-tight">{TEXTS.title}</h2>
    ...
    <div className="flex flex-wrap items-center justify-between gap-4 bg-white dark:bg-[#1e2433] p-4 rounded-xl border border-slate-200 dark:border-[#2a324a]">
      ...
      <div className="flex items-center gap-3">
        <button className="flex h-9 items-center gap-2 rounded-lg bg-slate-100 dark:bg-[#252d46] hover:bg-slate-200 dark:hover:bg-[#2f3956] border border-slate-200 dark:border-[#2a324a] px-3 transition-colors">
          <span className="text-slate-700 dark:text-white text-sm font-medium">{TEXTS.filterProject}</span>
    ...
    <div className="grid grid-cols-12 gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-[#252d46] transition-colors group items-start">
      ...
      <div className="col-span-3 flex flex-col gap-1.5">
        ...
        {Object.keys(entity.schema || {}).length === 0 && (
          <div className="text-[10px] text-slate-400 dark:text-[#95a0c6] italic">{TEXTS.table.noAttributes}</div>
        )}
      </div>
...
const ModalInternal: React.FC<ModalProps> = React.memo(({ isOpen, onClose, onSave, editingEntity }) => {
  const [activeTab, setActiveTab] = useState<'general' | 'attributes' | 'relationships'>('attributes');
  const [formData, setFormData] = useState({
  ...
  const [attributes, setAttributes] = useState<Attribute[]>(
    editingEntity
      ? Object.entries(editingEntity.schema || {}).map(([key, val]: [string, any]) => ({
          ...
          description: typeof val === 'string' ? '' : val.description || '',
          required: typeof val === 'string' ? false : !!val.required,
          validation: typeof val === 'string' ? {} : { ge: val.ge, le: val.le, min_len: val.min_len, max_len: val.max_len, regex: val.regex },
        }))
      : []
  );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white leading-none">
                {editingEntity ? `${TEXTS.modal.titleEdit} ${editingEntity.name}` : TEXTS.modal.titleNew}
              </h3>
          ...
          <div className="p-6 flex flex-col gap-8">
            {activeTab === 'general' && (
              <div className="flex flex-col gap-4">
                <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">{TEXTS.modal.basicInfo}</h4>
              ...
              <>
                <div className="bg-blue-50 dark:bg-blue-500/5 border border-blue-200 dark:border-blue-500/20 rounded-lg p-4 flex gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    <h4 className="text-sm font-bold text-blue-900 dark:text-blue-100">{TEXTS.modal.infoTitle}</h4>
                    <p className="text-xs text-blue-700 dark:text-blue-200/70">{TEXTS.modal.infoDesc}</p>
                  </div>
                </div>
            ...
            {activeTab === 'relationships' && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">{TEXTS.relationshipMapping}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-sm">{TEXTS.relationshipsComingSoon}</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="text-xs text-slate-500 dark:text-[#95a0c6] flex items-center gap-1">
            ...
            <span>
              {TEXTS.modal.lastSaved.replace(
                '{{time}}',
                editingEntity?.updated_at ? new Date(editingEntity.updated_at).toLocaleString() : TEXTS.modal.neverSaved
              )}
            </span>
          </div>
          <div className="flex items-center gap-3">
