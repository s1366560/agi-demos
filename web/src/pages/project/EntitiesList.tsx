...
const EntitiesListInner: React.FC<EntitiesListRootProps> = memo(({
  ...
  const paginationChild = childrenArray.find((child: any) => child?.type?.[PAGINATION_SYMBOL]) as any
  const detailChild = childrenArray.find((child: any) => child?.type?.[DETAIL_SYMBOL]) as any

  // Determine if using compound mode
  const hasSubComponents = headerChild || filtersChild || statsChild || listChild || paginationChild || detailChild
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      {includeFilters && (
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4" data-testid="entities-filters">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {/* Entity Type Filter */}
            <div className="flex items-center gap-2">
              <label htmlFor="entity-type-filter" className="text-sm font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">
              ...
              >
                <option value="">{t('project.graph.entities.filter.all_types')} ({totalCount})</option>
                {loadingTypes ? (
          ...
          {/* Stats */}
          {includeStats && (
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-200 dark:border-slate-700" data-testid="entities-stats">
              <div className="flex gap-4 text-sm text-slate-600 dark:text-slate-400">
                <span>
                  {t('project.graph.entities.stats.showing', { count: sortedEntities.length, total: totalCount.toLocaleString() })}
                </span>
                {entityTypeFilter && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {loading ? (
              ...
              <>
                ...
                {includePagination && totalCount > limit && (
                  <div className="flex items-center justify-between bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4" data-testid="entities-pagination">
                    <button
                    ...
                    <span className="text-sm text-slate-600 dark:text-slate-400">
                      {t('common.pagination.page_info', { page: page + 1, total: Math.ceil(totalCount / limit), defaultValue: `Page ${page + 1} of ${Math.ceil(totalCount / limit)}` })}
                    </span>
                    <button
        ...
        {includeDetail && (
          <div className="lg:col-span-1">
            {selectedEntity ? (
              <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6 sticky top-6" data-testid="entities-detail">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase">{t('project.graph.entities.detail.name')}</label>
                    <p className="text-slate-900 dark:text-white font-medium">{selectedEntity.name}</p>
                  </div>

                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase">{t('project.graph.entities.detail.type')}</label>
                  ...
                  {selectedEntity.summary && (
                    <div>
                      <label className="text-xs font-semibold text-slate-500 uppercase">{t('project.graph.entities.detail.summary')}</label>
                      <p className="text-sm text-slate-600 dark:text-slate-400">{selectedEntity.summary}</p>
                    </div>
                  )}

                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase">{t('project.graph.entities.detail.uuid')}</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase">{t('project.graph.entities.detail.created')}</label>
                    <p className="text-sm text-slate-600 dark:text-slate-400">
                      {selectedEntity.created_at ? new Date(selectedEntity.created_at).toLocaleString() : t('common.status.unknown', 'Unknown')}
                    </p>
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                      ...
                      {t('project.graph.entities.detail.relationships', { count: relationships.length })}
                    </h3>
                          ...
                          >
                            ...
                            <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                              <div className="text-xs text-slate-500 dark:text-slate-400">
                                {t('project.graph.entities.detail.related')} <span className="font-medium text-slate-700 dark:text-slate-300">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-12 text-center sticky top-6" data-testid="entities-detail">
                <span className="material-symbols-outlined text-4xl text-slate-400">
                  touch_app
                </span>
                <p className="text-slate-500 mt-2">{t('project.graph.entities.detail.select_prompt')}</p>
