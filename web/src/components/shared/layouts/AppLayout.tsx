...
import { Menu, X, Bell, LogOut, Brain, Search } from 'lucide-react';
...
export const AppLayout: React.FC<AppLayoutProps> = ({
  ...
  const finalGroups: NavigationGroup[] =
    navigationGroups.length > 0 ? navigationGroups : [{ items: navigationItems }];
  ...
  const NavItem = ({ item, isMobile = false }: { item: NavigationItem; isMobile?: boolean }) => {
    ...
    return (
      <button
        ...
        className={`flex items-center space-x-3 w-full px-4 py-2.5 text-sm font-medium rounded-lg transition-colors ${
          isActive
      ...
      >
        <Icon
          className={`h-5 w-5 ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-slate-500'}`}
        />
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#121520] flex flex-col lg:flex-row font-sans">
      ...
      {isMobileMenuOpen && (
        ...
        >
          <div
            className="fixed inset-y-0 left-0 w-64 bg-white dark:bg-slate-900 shadow-xl flex flex-col border-r border-gray-200 dark:border-slate-800"
        ...
        <div className="flex items-center h-16 px-6 border-b border-gray-100 dark:border-slate-800">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xl font-bold text-gray-900 dark:text-white">
              MemStack<span className="text-purple-600 dark:text-purple-400">.ai</span>
            </span>
          </div>
        </div>
