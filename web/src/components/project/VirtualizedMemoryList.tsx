...
export const MemoryRow = memo<MemoryRowProps>(({ memory, onClick, index, isFocused, onFocus }) => {
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-3 text-slate-600 dark:text-slate-300 text-right font-mono" role="gridcell">
...
export const VirtualizedMemoryList = memo<VirtualizedMemoryListProps>(({
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
    ...
    switch (e.key) {
      ...
      case 'PageDown':
        e.preventDefault()
        newIndex = Math.min(focusedIndex + Math.floor(containerHeight / estimateSize()), memories.length - 1)
        break
      case 'PageUp':
        e.preventDefault()
