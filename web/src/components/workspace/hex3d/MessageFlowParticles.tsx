...
export const MessageFlowParticles = forwardRef<MessageFlowParticlesRef, MessageFlowParticlesProps>(
  ({ agents, hexSize }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      triggerFlow: (fromAgentId: string, toAgentId: string) => {
        ...
        if (
          !fromAgent ||
          !toAgent ||
          fromAgent.hex_q === undefined ||
          fromAgent.hex_r === undefined ||
          toAgent.hex_q === undefined ||
          toAgent.hex_r === undefined
        ) {
          return;
        }
    ...
    return (
      <group>
        {flows.map((flow) => (
          <SingleFlow key={flow.id} flow={flow} hexSize={hexSize} onComplete={handleComplete} />
        ))}
      </group>
    );
  }
