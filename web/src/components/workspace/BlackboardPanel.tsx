...
export function BlackboardPanel({ tenantId, projectId, workspaceId }: BlackboardPanelProps) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 dark:border-slate-700 p-4 bg-white dark:bg-slate-800 transition-colors duration-200">
      <h3 className="font-semibold text-slate-900 dark:text-white mb-3">{t('workspaceDetail.blackboard.title')}</h3>
      ...
      <div className="space-y-3">
        {posts.map((post) => {
          ...
          return (
            <article key={post.id} className="border border-slate-200 dark:border-slate-700 rounded p-3 transition-all duration-300">
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('workspaceDetail.blackboard.status')}: {post.status}</div>

              {replies.length > 0 && (
                <div className="mt-3 space-y-2 pl-2 border-l-2 border-slate-100 dark:border-slate-700/50">
                  {replies.map((reply) => (
                    <div key={reply.id} className="text-sm text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-700/50 p-2 rounded">
                      {reply.content}
                    </div>
                  ))}
                </div>
