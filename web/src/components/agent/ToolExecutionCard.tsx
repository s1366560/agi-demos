...
import { CodeExecutorResultCard, parseCodeExecutorResult } from './CodeExecutorResultCard';
...
export const ToolExecutionCard = memo<ToolExecutionCardProps>(({ toolCall }) => {
  ...
  const webSearchResults =
    isWebSearch && hasResult && toolCall.result ? parseWebSearchResults(toolCall.result) : null;
  const webScrapeResults =
    isWebScrape && hasResult && toolCall.result ? parseWebScrapeResults(toolCall.result) : null;
  ...
  // Parse image results
  const imageUrl = hasResult && toolCall.result ? extractImageUrl(toolCall.result) : null;
  const base64Image = hasResult && toolCall.result ? parseBase64Image(toolCall.result) : null;
  ...
  return (
    <Card
      ...
      style={{
        marginBottom: 8,
        backgroundColor:
          status === 'completed' ? '#f6ffed' : status === 'failed' ? '#fff1f0' : '#e6f7ff',
        border: `1px solid ${
          status === 'completed' ? '#b7eb8f' : status === 'failed' ? '#ffccc7' : '#91d5ff'
        }`,
      }}
    ...
    >
      <Space orientation="vertical" size="small" style={{ width: '100%' }}>
        ...
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          ...
          {(toolCall.result || hasError || Object.keys(toolCall.input).length > 0) && (
            <Typography.Link
          ...
          <Space orientation="vertical" size="small" style={{ width: '100%' }}>
            ...
            {hasError ? (
              ...
              <>
                ...
                {codeExecutorResults && <CodeExecutorResultCard result={codeExecutorResults} />}
