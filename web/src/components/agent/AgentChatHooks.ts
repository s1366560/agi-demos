...
export function useAgentChatHandlers(options: {
  ...
  sendMessage: (content: string, projectId: string, handlers: { onAct: unknown; onObserve: unknown }) => Promise<string | undefined>
  onAct: unknown
  onObserve: unknown
  ...
  const handlePanelClose = useCallback((setPanelCollapsed: (value: boolean | ((prev: boolean) => boolean)) => void) => {
