...
const MODEL_PRESETS: Record<ProviderType, {
    ...
    openai: {
        ...
        reranker: [
            { name: 'gpt-4o-mini', description: 'LLM-based reranking' },
        ],
    },
    anthropic: {
        llm: [
        ...
        embedding: [],
        reranker: [
            { name: 'claude-3-5-haiku-20241022', description: 'LLM-based reranking' },
        ],
    },
    gemini: {
        llm: [
        ...
        embedding: [
            { name: 'text-embedding-004', dimension: 768 },
        ],
        reranker: [
            { name: 'gemini-1.5-flash', description: 'LLM-based reranking' },
        ],
    },
    qwen: {
        llm: [
        ...
        small: [
            { name: 'qwen-turbo', description: 'Fast & cost-effective' },
        ],
        embedding: [
    ...
    deepseek: {
        ...
        small: [
            { name: 'deepseek-coder', description: 'Code specialized' },
        ],
        embedding: [],
        reranker: [
            { name: 'deepseek-chat', description: 'LLM-based reranking' },
        ],
    },
    zai: {
        llm: [
        ...
        reranker: [
            { name: 'glm-4-flash', description: 'LLM-based reranking' },
        ],
    },
    cohere: {
        llm: [
    ...
    mistral: {
        ...
        embedding: [
            { name: 'mistral-embed', dimension: 1024 },
        ],
        reranker: [
            { name: 'mistral-small-latest', description: 'LLM-based reranking' },
        ],
    },
    groq: {
        llm: [
    ...
    bedrock: {
        ...
        small: [
            { name: 'anthropic.claude-3-haiku-20240307-v1:0', description: 'Fast Claude' },
        ],
        embedding: [
    ...
    vertex: {
        llm: [
            { name: 'gemini-1.5-pro', description: 'Gemini Pro on Vertex' },
        ],
        small: [
            { name: 'gemini-1.5-flash', description: 'Gemini Flash on Vertex' },
        ],
        embedding: [
            { name: 'textembedding-gecko', dimension: 768 },
        ],
        reranker: [],
    },
}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500">
                                Configure via LiteLLM unified interface
                            </p>
                        </div>
                    </div>
                        ...
                        {steps.map((step, index) => {
                            ...
                            return (
                                <React.Fragment key={step.key}>
                                    ...
                                    >
                                        <span className={`material-symbols-outlined text-lg ${isPast && !isActive ? 'text-green-500' : ''}`}>
                                            {isPast && !isActive ? 'check_circle' : step.icon}
                                        </span>
                                        <span className="text-sm font-medium hidden sm:inline">{step.label}</span>
                                    </button>
                                    {index < steps.length - 1 && (
                                        <div className={`flex-1 h-0.5 mx-2 ${isPast ? 'bg-primary' : 'bg-slate-200 dark:bg-slate-700'}`} />
                                    )}
                                </React.Fragment>
                            )
                        })}
                    ...
                    {currentStep === 'provider' && (
                        <div className="space-y-4">
                            <p className="text-slate-600 dark:text-slate-400 mb-4">
                                Select your LLM provider. All providers are accessed through LiteLLM's unified interface.
                            </p>
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="font-medium text-slate-900 dark:text-white">{provider.label}</div>
                                                <div className="text-xs text-slate-500 mt-1 line-clamp-2">{provider.description}</div>
                                                <div className="flex gap-1 mt-2">
                                                    {provider.hasEmbedding && (
                                                        <span className="px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-[10px] rounded">Embed</span>
                                                    )}
                                                    {provider.hasNativeRerank && (
                                                        <span className="px-1.5 py-0.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 text-[10px] rounded">Rerank</span>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                    ...
                    {currentStep === 'credentials' && selectedProvider && (
                        <div className="space-y-6">
                            <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                                ...
                                <div>
                                    <div className="font-semibold text-slate-900 dark:text-white">{selectedProvider.label}</div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        placeholder={isEditing ? 'Leave empty to keep current' : selectedProvider.apiKeyPlaceholder}
                                ...
                                <p className="mt-1.5 text-xs text-slate-500 flex items-center gap-1">
                                    ...
                                    Environment variable: <code className="px-1 bg-slate-100 dark:bg-slate-700 rounded">{selectedProvider.apiKeyEnvVar}</code>
                                </p>
                                {testResult && (
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 gap-2">
                                    {presets.llm.map(model => (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <code className="text-sm font-mono text-slate-900 dark:text-white">{model.name}</code>
                                                {formData.llm_model === model.name && (
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {presets.small.map(model => (
                                            <button
                                                key={model.name}
                                                type="button"
                                                onClick={() => setFormData(prev => ({ ...prev, llm_small_model: model.name }))}
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {presets.embedding.map(model => (
                                            <button
                                                key={model.name}
                                                type="button"
                                                onClick={() => setFormData(prev => ({ ...prev, embedding_model: model.name }))}
                                                className={`p-3 rounded-lg border text-left transition-all ${
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {presets.reranker.map(model => (
                                            <button
                                                key={model.name}
                                                type="button"
                                                onClick={() => setFormData(prev => ({ ...prev, reranker_model: model.name }))}
                                                className={`px-3 py-2 rounded-lg border text-sm transition-all ${
                                            ...
                                            >
                                                ...
                                                {model.description.includes('Native') && (
                                                    <span className="ml-1 text-[10px] bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 px-1 rounded">Native</span>
                                                )}
                                            </button>
                                        ))}
                                    </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{formData.name}</h3>
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-4">
                                    <div>
                                        ...
                                        <code className="text-sm font-mono text-slate-900 dark:text-white">{formData.llm_model}</code>
                                    </div>
                                    {formData.llm_small_model && (
                                        <div>
                                            <p className="text-xs text-slate-500 uppercase tracking-wider">Small Model</p>
                                            <code className="text-sm font-mono text-slate-900 dark:text-white">{formData.llm_small_model}</code>
                                        </div>
                                    )}
                                    {formData.embedding_model && (
                                        <div>
                                            <p className="text-xs text-slate-500 uppercase tracking-wider">Embedding Model</p>
                                            <code className="text-sm font-mono text-slate-900 dark:text-white">{formData.embedding_model}</code>
                                        </div>
                                    )}
                                    {formData.reranker_model && (
                                        <div>
                                            <p className="text-xs text-slate-500 uppercase tracking-wider">Reranker Model</p>
                                            <code className="text-sm font-mono text-slate-900 dark:text-white">{formData.reranker_model}</code>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center gap-4 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                    <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${
                                        ...
                                        <span className={`h-1.5 w-1.5 rounded-full ${formData.is_active ? 'bg-green-500' : 'bg-slate-400'}`} />
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="text-sm text-blue-800 dark:text-blue-200">
                                        ...
                                        <p className="mt-1 text-blue-700 dark:text-blue-300">
                                            This provider will be accessible through LiteLLM's unified API. 
                                            Model names are automatically prefixed for correct routing.
                                        </p>
                                    </div>
                                </div>
                            </div>
