...
export const GraphVisualization: React.FC<GraphVisualizationProps> = ({
  ...
  return (
    <div className="bg-[#111521] rounded-lg shadow-sm border border-slate-800 relative h-full flex flex-col overflow-hidden" ref={containerRef}>
      {/* Grid Pattern Background */}
      <div className="absolute inset-0 opacity-20 pointer-events-none" 
           style={{ backgroundImage: 'radial-gradient(#2b324a 1px, transparent 1px)', backgroundSize: '40px 40px' }}>
      </div>
      ...
      <div className="absolute top-6 left-6 flex flex-col gap-2 z-10">
        ...
        <div className="bg-[#1e2332] border border-slate-700 rounded-lg shadow-xl overflow-hidden flex flex-col mt-2">
            <button onClick={handleZoomIn} className="p-2.5 text-slate-400 hover:text-white hover:bg-slate-700 border-b border-slate-700 transition-colors">
                <Plus className="w-5 h-5" />
            </button>
            <button onClick={handleZoomOut} className="p-2.5 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors">
                <Minus className="w-5 h-5" />
            </button>
        </div>
        <button onClick={handleResetView} className="bg-[#1e2332] border border-slate-700 rounded-lg shadow-xl p-2.5 text-slate-400 hover:text-white hover:bg-slate-700 mt-2 transition-colors">
            ...
            <div className="flex flex-col gap-2">
                {availableTypes.map(type => (
                    <label key={type} className="flex items-center gap-2 text-sm text-slate-300 cursor-pointer hover:text-white">
                        ...
                        />
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: getNodeColor(type) }}></span>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
                <div>
                    <div className="flex justify-between items-end mb-1">
                        ...
                        <span className="text-emerald-400 font-bold text-sm">{(selectedNode.entity.importance || 1) * 10}/100</span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
                        <div className="bg-gradient-to-r from-emerald-500 to-blue-600 h-full rounded-full" style={{ width: `${(selectedNode.entity.importance || 1) * 10}%` }}></div>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div className="space-y-2">
                        {Object.entries(selectedNode.entity.metadata || {}).map(([key, val]: [string, any]) => (
