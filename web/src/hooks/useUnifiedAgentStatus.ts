 ...
 */
function deriveToolStats(lifecycleData: LifecycleStateData | null): ToolStats {
  return {
 ...
 */
function deriveSkillStats(lifecycleData: LifecycleStateData | null): SkillStats {
  return {
...
/**
 * Derive connection status from WebSocket and sandbox states
 *
 * @param isWebSocketConnected - Actual WebSocket connection state from useAgentLifecycleState
 * @param activeSandboxId - Active sandbox ID from sandbox store
 */
...
export function useUnifiedAgentStatus({
  ...
  const {
    lifecycleState,
    isConnected: isLifecycleConnected,
