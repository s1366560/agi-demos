...
export function useConversationStatuses() {
  ...
  const getConversationsWithPendingHITL = useAgentV3Store(
    (state) => state.getConversationsWithPendingHITL
  );
  const getStreamingConversationCount = useAgentV3Store(
    (state) => state.getStreamingConversationCount
  );
