...
export default class RFB extends EventTargetMixin {
    ...
    */
    ...
    _fail(details) {
        ...
        this._rfbDisconnectReason = details; // Store reason for disconnect event
    ...
    _normalMsg() {
        ...
        switch (msgType) {
            ...
            case 184: // KASM video encoders (KasmVNC 1.4.0+)
                return this._handleServerVideoEncoders();

            case 185: // KASM keep alive (KasmVNC 1.4.0+)
                return true; // No payload, just acknowledge

            case 186: // KASM server disconnect notice (KasmVNC 1.4.0+)
            ...
            case 253: // KASM user joined shared session (KasmVNC 1.4.0+)
                return this._handleUserJoin();

            case 254: // KASM user left shared session (KasmVNC 1.4.0+)
    ...
    _handleServerVideoEncoders() {
        ...
        for (let i = 0; i < num; i++) {
            if (this._sock.rQwait("VideoEncoders entry", 13)) return false;
            this._sock.rQshift32(); // codec
            this._sock.rQshift32(); // minQuality
            this._sock.rQshift32(); // maxQuality
    ...
    _handleDisconnectNotify() {
        ...
        Log.Info("Server disconnect notice: graceful=" + graceful + " reason=" + reason);
    ...
    _handleUserJoin() {
        ...
        const text = this._sock.rQshiftStr(length);
        Log.Info("User joined shared session: " + text);
        return true;
    }
    ...
    _handleUserLeft() {
        ...
        const text = this._sock.rQshiftStr(length);
        Log.Info("User left shared session: " + text);
        return true;
    }
