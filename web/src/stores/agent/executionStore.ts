...
export interface ExecutionState {
  ...
  completeTool: (callId: string | undefined, observation: string) => void;
...
function isObservationError(observation: string): boolean {
  ...
  return lowerObs.startsWith('error:') || lowerObs.includes('failed');
}
...
export const useExecutionStore = create<ExecutionState>()(
   ...
   */
  completeTool: (callId: string | undefined, observation: string) => {
    const endTime = new Date().toISOString();
    ...
    set((state) => {
      ...
      const isError = isObservationError(observation);
      ...
      const targetStepNumber = targetExecution?.stepNumber ?? state.currentToolExecution?.stepNumber;
...
export const useMatchedPattern = () =>
  useExecutionStore((state) => state.matchedPattern);
