        ...
        sendMessage: async (content, projectId, additionalHandlers) => {
          ...
          const handler: AgentStreamHandler = {
            ...
            onThoughtDelta: (event) => {
              ...
              if (!buffer.thoughtDeltaFlushTimer) {
                buffer.thoughtDeltaFlushTimer = setTimeout(() => {
                  ...
                  if (bufferedContent) {
                    const { activeConversationId, updateConversationState, getConversationState } =
                      get();
            ...
            onWorkPlan: (event) => {
              ...
              const workPlanEvent: AgentEvent<WorkPlanEventData> =
                event as AgentEvent<WorkPlanEventData>;
              const convState = getConversationState(handlerConversationId);
            ...
            onPlanExecutionComplete: (event) => {
              ...
              const updatedExecutionPlan = convState.executionPlan
                ? {
                    ...
                    progress_percentage:
                      (eventData.completed_steps || 0) /
                      (convState.executionPlan.steps.length || 1),
              ...
              if (!buffer.textDeltaFlushTimer) {
                buffer.textDeltaFlushTimer = setTimeout(() => {
                  ...
                  if (bufferedContent) {
                    ...
                    updateConversationState(handlerConversationId, {
                      streamingAssistantContent:
                        convState.streamingAssistantContent + bufferedContent,
                      streamStatus: 'streaming',
                    });
                    ...
                    if (handlerConversationId === activeConversationId) {
                      ...
                      set((state) => ({
                        streamingAssistantContent:
                          state.streamingAssistantContent + bufferedContent,
                        streamStatus: 'streaming',
                      }));
                    } else {
                      console.log(
            ...
            onTextEnd: (event) => {
              ...
              const finalContent =
                fullText || convState.streamingAssistantContent + remainingBuffer;
