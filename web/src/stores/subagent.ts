...
export const useCurrentSubAgent = () =>
  useSubAgentStore((state) => state.currentSubAgent);
export const useSubAgentTemplates = () =>
  useSubAgentStore((state) => state.templates);
export const useSubAgentTotal = () => useSubAgentStore((state) => state.total);

// Filter selectors
export const useSubAgentFilters = () =>
  useSubAgentStore((state) => state.filters);

// Loading selectors
export const useSubAgentLoading = () =>
  useSubAgentStore((state) => state.isLoading);
export const useSubAgentTemplatesLoading = () =>
  useSubAgentStore((state) => state.isTemplatesLoading);
export const useSubAgentSubmitting = () =>
  useSubAgentStore((state) => state.isSubmitting);
...
export const useSubAgentData = () =>
  useSubAgentStore((state) => state.subagents);
export const useSubAgentFiltersData = () =>
  useSubAgentStore((state) => state.filters);
...
export const useEnabledSubAgentsCount = () =>
  useSubAgentStore(
    (state) => state.subagents.filter((sa) => sa.enabled).length
  );

export const useAverageSuccessRate = () =>
  useSubAgentStore((state) => {
    const enabledAgents = state.subagents.filter(
      (sa) => sa.enabled && sa.total_invocations > 0
    );
    if (enabledAgents.length === 0) return 0;
    const totalRate = enabledAgents.reduce(
      (sum, sa) => sum + sa.success_rate,
      0
    );
...
export const useTotalInvocations = () =>
  useSubAgentStore((state) =>
    state.subagents.reduce((sum, sa) => sum + sa.total_invocations, 0)
  );

// Action selectors - each returns a stable function reference
export const useListSubAgents = () =>
  useSubAgentStore((state) => state.listSubAgents);
export const useGetSubAgent = () =>
  useSubAgentStore((state) => state.getSubAgent);
export const useCreateSubAgent = () =>
  useSubAgentStore((state) => state.createSubAgent);
export const useUpdateSubAgent = () =>
  useSubAgentStore((state) => state.updateSubAgent);
export const useDeleteSubAgent = () =>
  useSubAgentStore((state) => state.deleteSubAgent);
export const useToggleSubAgent = () =>
  useSubAgentStore((state) => state.toggleSubAgent);
export const useSetCurrentSubAgent = () =>
  useSubAgentStore((state) => state.setCurrentSubAgent);
export const useListTemplates = () =>
  useSubAgentStore((state) => state.listTemplates);
export const useCreateFromTemplate = () =>
  useSubAgentStore((state) => state.createFromTemplate);
export const useSetSubAgentFilters = () =>
  useSubAgentStore((state) => state.setFilters);
export const useResetSubAgentFilters = () =>
  useSubAgentStore((state) => state.resetFilters);
export const useClearSubAgentError = () =>
  useSubAgentStore((state) => state.clearError);
export const useResetSubAgentStore = () =>
  useSubAgentStore((state) => state.reset);
