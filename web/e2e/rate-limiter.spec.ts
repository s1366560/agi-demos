...
async function getOrCreateProject(page: any, authToken: string) {
  const projectResult = await page.evaluate(
    async (params: { apiUrl: string; token: string }) => {
...
test.describe('Rate Limiter - Agent Worker', () => {
  test('should handle concurrent conversation creation requests', async ({ page }) => {
    ...
    const results = await page.evaluate(
      async (params: { apiUrl: string; token: string; pid: string; count: number }) => {
  ...
  test('should handle burst requests without 429 errors', async ({ page }) => {
    ...
    const results = await page.evaluate(
      async (params: { apiUrl: string; token: string; count: number }) => {
...
test.describe('Rate Limiter - Sequential Processing', () => {
  test('should handle sequential requests correctly', async ({ page }) => {
    ...
    expect(
      ...
      'No project available for sequential rate limiter test'
    ).toBeTruthy();
    ...
    for (let i = 0; i < 3; i++) {
      const result = await page.evaluate(
        async (params: { apiUrl: string; token: string; pid: string; idx: number }) => {
