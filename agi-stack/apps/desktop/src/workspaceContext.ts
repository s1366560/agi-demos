import type { TaskMode } from './data';
...
export const tenantCatalog: Tenant[] = [
  {
    id: 'northstar',
    name: 'Northstar Labs',
    shortName: 'Northstar',
...
export function getProject(tenantId: string, projectId: string): Project {
  const tenant = getTenant(tenantId);
  return tenant.projects.find((project) => project.id === projectId) ?? tenant.projects[0];
}

export function getProjectWorkspaces(tenantId: string, projectId: string): Workspace[] {
...
export function getWorkspace(tenantId: string, projectId: string, workspaceId: string): Workspace {
