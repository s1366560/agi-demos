...
fn local_router(state: Arc<LocalRuntimeState>) -> Router {
    let protected = Router::new()
        ...
        .route("/api/v1/agent/runs/:run_id/pause", post(pause_run))
        .route("/api/v1/agent/runs/:run_id/resume", post(resume_run))
        .route("/api/v1/agent/runs/:run_id/fork", post(fork_recovery_run))
        .route("/api/v1/agent/runs/:run_id/cancel", post(cancel_run))
        .route("/api/v1/agent/runs/:run_id/review", post(review_run))
        .route(
...
#[derive(Deserialize)]
struct ForkRecoveryRunBody {
    expected_revision: u64,
    idempotency_key: String,
}

async fn fork_recovery_run(
    State(state): State<Arc<LocalRuntimeState>>,
    Extension(authenticated): Extension<AuthenticatedContext>,
    Path(run_id): Path<String>,
