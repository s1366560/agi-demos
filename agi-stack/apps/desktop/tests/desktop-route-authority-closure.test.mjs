...
test('production registry and runtime snapshot close implemented route ownership in both directions', () => {
...
test(
  'runtime snapshot declares tenant workspaces and canonical Agent Workspace authority',
  async () => {
    ...
    try {
      ...
      assert.deepEqual(snapshot.capabilities['tenant-tenant-workspaces'], {
        ...
        reason_code: 'local_workspace_lifecycle_partial',
        service_version: '0.1.0',
        contract_version: '3.0.0',
        allowed_actions: ['view', 'list', 'create'],
        scope: {
          tenant_id: 'local',
