...
test('DesktopCapabilitySnapshot v2 remains a validated read-only input contract', () => {
  ...
  assert.deepEqual(normalizeWebParityFixture(input), fixture.web_expected_view_model);
  assert.deepEqual(normalizeDesktopParityFixture(input), fixture.desktop_expected_view_model);
});
...
test('DesktopCapabilitySnapshot v2 rejects legacy and internally inconsistent snapshots', () => {
  ...
  assert.equal(parseDesktopCapabilitySnapshot(degradedWithoutServiceVersion), null);
...
test('capability contract negotiation accepts compatible versions and fails old services closed', () => {
  assert.deepEqual(
    negotiateCapabilityContract({ service_version: '0.1.0', contract_version: '2.3.0' }, '2.0.0'),
    {
      compatible: true,
      reason_code: null,
      service_version: '0.1.0',
  ...
  assert.deepEqual(
    negotiateCapabilityContract({ service_version: '0.1.0', contract_version: '1.9.0' }, '2.0.0'),
    {
      compatible: false,
  ...
  assert.deepEqual(
    negotiateCapabilityContract({ service_version: '0.1.0', contract_version: '3.0.0' }, '2.0.0'),
    {
      compatible: false,
