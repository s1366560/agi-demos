...
const {
  ...
  activityEntriesToAuthorityReceipts,
...
test("Activity inbox prefers the canonical nested run completion summary", () => {
  ...
  assert.equal(entry.subtitle, "Canonical completion summary");
});

test("Activity authority receipts bind entry revisions and ISO read timestamps", () => {
  const entries = buildActivityInboxEntries([
    { ...baseItem, authority_kind: "agent_run", revision: 7 },
    { ...items[1], revision: null },
  ]);
  const receipts = activityEntriesToAuthorityReceipts(
    entries,
  ...
  assert.deepEqual(receipts, [
    {
      ...
      entry_revision: 7,
      read_at: "2026-08-04T02:00:00.000Z",
    },
    {
      entry_id: "hitl_request:hitl-1",
      entry_revision: 0,
      read_at: "2026-08-04T02:00:00.000Z",
    },
  ]);
  assert.deepEqual(activityAuthorityEntriesToReadState(receipts), {
    "agent_run:run-1": Date.parse("2026-08-04T02:00:00Z"),
    "hitl_request:hitl-1": Date.parse("2026-08-04T02:00:00Z"),
  });
});
...
test("Activity inbox never infers categories from narrative titles", () => {
  const decoys = [
    {
      ...baseItem,
      authority_id: "decoy-1",
      title: "Failed while running",
      group: "ready_review",
      status: "ready_review",
    },
    {
      ...baseItem,
      authority_id: "decoy-2",
      title: "Ready for review now",
      group: "running",
      status: "running",
    },
  ];
...
test("Marking a conversation read only touches that conversation entries", () => {
  ...
  const unchanged = markActivityConversationRead(
    next,
    entries,
