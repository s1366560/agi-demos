...
test('terminal run with no evidence renders outcome only and omits empty sections', () => {
  const summary = buildRunCompletionSummary({ status: 'completed', capabilityMode: 'code' });
...
test('failed and cancelled runs surface the failure reason truthfully', () => {
  ...
  const cancelled = buildRunCompletionSummary({ status: 'cancelled', capabilityMode: 'work' });
...
test('changes section requires a ready snapshot with files and links to the Changes canvas', () => {
  ...
  // Work mode has no Changes canvas tab: stats remain, but no dead link.
  ...
  assert.equal(workMode.changes.link, null);
});
...
test('myWorkCompletionPresentation only appears for the ready-review group', () => {
  assert.equal(
    myWorkCompletionPresentation({ group: 'running', status: 'running', error: null }),
    null,
  );
  assert.equal(
    myWorkCompletionPresentation({ group: 'needs_input', status: 'needs_input', error: null }),
    null,
  );

  assert.deepEqual(
    myWorkCompletionPresentation({ group: 'ready_review', status: 'ready_review', error: null }),
    { outcomeLabelKey: 'myWork.status.ready_review', tone: 'success', detail: null },
  );
  assert.deepEqual(
    myWorkCompletionPresentation({ group: 'ready_review', status: 'completed', error: null }),
    { outcomeLabelKey: 'myWork.status.completed', tone: 'success', detail: null },
  );
  assert.deepEqual(
    myWorkCompletionPresentation({
      group: 'ready_review',
    ...
    { outcomeLabelKey: 'myWork.status.failed', tone: 'danger', detail: 'tests red' },
  );
  assert.deepEqual(
    myWorkCompletionPresentation({ group: 'ready_review', status: 'cancelled', error: null }),
    { outcomeLabelKey: 'myWork.status.cancelled', tone: 'warning', detail: null },
  );
  // A completed run stale-grouped as running still lands in ready review.
  assert.deepEqual(
    myWorkCompletionPresentation({ group: 'running', status: 'completed', error: null }),
    { outcomeLabelKey: 'myWork.status.completed', tone: 'success', detail: null },
  );
});
...
test('usage passes through verbatim and defaults to null', () => {
  const usage = { currentTokens: 12_300, tokenBudget: 200_000, occupancyPct: 6.2 };
  const summary = buildRunCompletionSummary({
    status: 'failed',
    capabilityMode: 'code',
    usage,
  ...
  const without = buildRunCompletionSummary({ status: 'completed', capabilityMode: 'code' });
