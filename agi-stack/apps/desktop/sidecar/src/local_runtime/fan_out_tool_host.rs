...
/// Tools are the ordered union of the inner hosts' tools (first host wins on
/// duplicates); a call dispatches to the first host that lists the tool name.
...
impl ToolHost for FanOutToolHost {
    ...
    async fn call(&self, tool: &str, input_json: &str) -> CoreResult<String> {
        for host in &self.hosts {
            if host.list_tools().iter().any(|name| name == tool) {
