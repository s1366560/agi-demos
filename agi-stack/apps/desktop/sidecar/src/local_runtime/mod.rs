...
async fn desktop_proxy() -> Html<&'static str> {
    Html(
        r#"<!doctype html><html><body style="margin:0;background:#111;color:#ddd;font:14px system-ui;display:grid;place-items:center;height:100vh"><div>Local desktop mode uses the native Electron window.</div></body></html>"#,
    )
}
...
mod tests {
    ...
    #[tokio::test]
    async fn runtime_selection_route_allows_electron_put_preflight() {
    ...
    #[tokio::test]
    async fn cors_allows_electron_origin_and_rejects_web_origin() {
        let app = local_router(test_state("launch-secret"));
        let get_preflight = app
            .clone()
            .oneshot(
                Request::builder()
                    .method(Method::OPTIONS)
                    ...
                    .header(
                        "access-control-request-headers",
                        "authorization,x-agistack-launch",
                    )
                    .body(Body::empty())
                    .expect("authenticated GET preflight request"),
            )
            .await
            .expect("authenticated GET preflight response");
        assert_eq!(get_preflight.status(), axum::http::StatusCode::OK);
        assert_eq!(
            get_preflight
                .headers()
        ...
        let rejected_preflight = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::empty())
                    .expect("rejected GET preflight request"),
            )
            .await
            .expect("rejected GET preflight response");
