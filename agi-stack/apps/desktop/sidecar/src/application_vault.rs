        ...
        Err(error) if error.kind() == ErrorKind::NotFound => {
            ...
            match open_new_private_file(master_key_path) {
                Ok(mut file) => {
                    ...
                    {
                        drop(file);
                        remove_new_private_file(master_key_path);
                        master_key.zeroize();
                    ...
                    drop(file);
                    if sync_parent_directory(master_key_path).is_err() {
                        remove_new_private_file(master_key_path);
                        master_key.zeroize();
                        return Err(ApplicationVaultError::Unavailable);
                    }
...
fn ensure_private_database_file(path: &Path) -> Result<(), ApplicationVaultError> {
    let created = match open_new_private_file(path) {
        Ok(file) => {
    ...
    if created && sync_parent_directory(path).is_err() {
        remove_new_private_file(path);
        return Err(ApplicationVaultError::Unavailable);
    }
    Ok(())
...
fn remove_new_private_file(path: &Path) {
    let _ = fs::remove_file(path);
    let _ = sync_parent_directory(path);
}

fn sync_parent_directory(path: &Path) -> Result<(), ApplicationVaultError> {
    let parent = path.parent().ok_or(ApplicationVaultError::Unavailable)?;
...
mod tests {
    ...
    use super::{directory_sync_is_unsupported, ApplicationCredentialVault, ApplicationVaultError};
    ...
    #[cfg(unix)]
    #[test]
    fn reopening_vault_repairs_existing_master_key_permissions() {
        use std::os::unix::fs::PermissionsExt;

        let root = test_root("repair-master-key-permissions");
        let vault = ApplicationCredentialVault::open(&root).expect("create application vault");
        drop(vault);
        let master_key_path = root.join("credential-vault/master.key");
        fs::set_permissions(&master_key_path, fs::Permissions::from_mode(0o644))
            .expect("weaken fixture master key permissions");

        let reopened = ApplicationCredentialVault::open(&root).expect("reopen application vault");

        assert_eq!(
            fs::metadata(&master_key_path)
                .expect("read repaired master key metadata")
                .permissions()
                .mode()
                & 0o777,
            0o600
    ...
    fn directory_sync_ignores_only_explicitly_unsupported_errors() {
        assert!(directory_sync_is_unsupported(&std::io::Error::from(
        ...
        assert_eq!(
            directory_sync_is_unsupported(&std::io::Error::from(
                std::io::ErrorKind::PermissionDenied
            )),
            cfg!(windows)
        );
        assert!(!directory_sync_is_unsupported(&std::io::Error::other(
            "unexpected directory sync failure"
        )));
    }
}
