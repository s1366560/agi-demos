...
impl TerminalHub {
    ...
    pub(super) fn mark_lost(&self, code: &'static str) {
        if self.alive.swap(false, Ordering::AcqRel) {
            ...
            self.cancellation.send_replace(Some(code));
        }
    }
    ...
    pub(super) async fn attach(self: Arc<Self>, socket: WebSocket, after_sequence: u64) {
        ...
        let initial_lost = *cancellation.borrow();
        if let Some(code) = initial_lost {
            let _ = send_terminal_lost(&mut client_tx, code).await;
                ...
                tokio::select! {
                    changed = cancellation.changed() => {
                        let code = if changed.is_ok() {
                            (*cancellation.borrow()).unwrap_or("terminal_session_lost")
                        } else {
...
async fn send_terminal_lost<S>(client_tx: &mut S, code: &'static str) -> Result<(), axum::Error>
where
...
mod tests {
    ...
    #[tokio::test]
    async fn attach_after_loss_reports_canonical_loss_without_claiming_connected() {
        let (input, _input_rx) = mpsc::channel(1);
        let hub = TerminalHub::for_test(input, test_environment());
        hub.finalize_lost("terminal_authority_revoked").await;
        let url = spawn_hub_ws(Arc::clone(&hub), 0).await;
        ...
        let (mut client, _) = tokio_tungstenite::connect_async(url)
            .await
            .expect("connect lost hub websocket");
        let lost = next_json(&mut client).await;
        assert_eq!(lost["type"], "terminal_authority_revoked");
        assert_eq!(lost["refetch"], true);
    }

    #[tokio::test]
    async fn attached_client_observes_loss_through_durable_watch_state() {
        let (input, _input_rx) = mpsc::channel(1);
        let hub = TerminalHub::for_test(input, test_environment());
        let url = spawn_hub_ws(Arc::clone(&hub), 0).await;
        let (mut client, _) = tokio_tungstenite::connect_async(url)
        ...
        // The watch channel is the durable in-process termination authority;
        // broadcast delivery remains only a latency optimization.
        let _ = hub.cancellation.send(Some("terminal_session_lost"));
        let lost = next_json(&mut client).await;
        assert_eq!(lost["type"], "terminal_session_lost");
        assert_eq!(lost["refetch"], true);
    }
