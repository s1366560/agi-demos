...
本目录是 **新架构的根**。架构文档在 [`docs/`](docs/);**Phase 1 生产级 Cargo workspace 已落地于此根下**([`crates/`](crates/) + [`apps/`](apps/),`cargo test` 468 测试绿、`agistack-server` 全端点 curl 验证 —— 见下方[「现状」](#现状)与[「构建与运行」](#构建与运行phase-1-工作区))。
...
- 最新 P5 增量:Rust skill evolution pipeline executor foundation 已落地:worker 现在可串起 Python scheduler 的 summarize→judge→aggregate→evolve→job/cleanup 数据面,subjective LLM 阶段仍通过 server-only `SkillEvolutionStageEngine` 注入,默认生产 readiness gate 不消费队列;完整 LLM-backed stage engine 仍后续;见 [04 #295](docs/architecture/04-spike-evidence.md)。此前 scheduler data-plane foundation 见 [04 #294](docs/architecture/04-spike-evidence.md),worker queue foundation 见 [04 #293](docs/architecture/04-spike-evidence.md),run admission/gateway 见 [04 #292](docs/architecture/04-spike-evidence.md)。
...
make ci          # 最小门禁:整工作区测试(566 绿)+ 核心 wasm32
