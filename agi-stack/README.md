...
- 最新 P5 增量:Rust skill evolution run scheduler/gateway 已落地:`POST /api/v1/skills/evolution/run` 与 `POST /api/v1/skills/{skill_id}/evolution/run` 走 production `PgSkillEvolutionScheduler`,以 additive `agistack_skill_evolution_runs` 队列做活跃 run coalescing,并精确翻 gateway;见 [04 #292](docs/architecture/04-spike-evidence.md)。
...
make ci          # 最小门禁:整工作区测试(558 绿)+ 核心 wasm32
