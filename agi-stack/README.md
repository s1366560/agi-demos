...
- **Phase 0(进行中)**:决策 Spike + 架构文档。可移植核心与插件宿主两条主轴均已用可运行、可测试的 Rust Spike **证伪通过**(见 `spikes/rust-portable-core/`,仓库根)。
- 本架构文档由会话规划稿 `plan.md` 与 `rust-spike-plan.md` 整理而来,是后续 Phase 1+ 实现的权威依据。

## 参考

- 决策 Spike 代码与实测:仓库根 `spikes/rust-portable-core/`(`README.md` 含 8 项验证结论表)。
