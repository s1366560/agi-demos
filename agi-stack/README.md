...
- 最新 P5 增量:Rust skill evolution scheduler data-plane foundation 已落地:Postgres repo 已覆盖 Python scheduler 的 unprocessed/unscored session selection、summary/score writeback、scored group aggregation 与 exact session-batch duplicate job lookup;完整 LLM summarizer/judge/aggregator/evolution engine 仍后续;见 [04 #294](docs/architecture/04-spike-evidence.md)。此前 worker queue foundation 见 [04 #293](docs/architecture/04-spike-evidence.md),run admission/gateway 见 [04 #292](docs/architecture/04-spike-evidence.md)。
...
make ci          # 最小门禁:整工作区测试(562 绿)+ 核心 wasm32
