...
> **✅ 已落地(证据 [04 #28](04-spike-evidence.md))**:上表 **FailureRate 退避重启**行的 `DoomLoopDetector` + 成本核算已从"仅 `max_rounds`"升为完整的**结构触发器 + Agent-First 裁决**闭环。`crates/core/src/agent/doom_loop.rs`:`DoomLoopDetector`(滑窗内重复 `(tool,input)` 计数)+ `CostTracker`(rounds/tool-calls/tokens 预算算术)为**确定性触发器**,只点火 `TriggerReason::{DoomLoop, CostCeiling}`;主观 verdict 经注入端口 `SupervisorPort::review → {health, next, rationale}` 由 **agent 工具调用**做出,引擎再确定性行动(continue/escalate/reassign)。**这是仓库顶层 Agent First 铁律(结构 trigger 确定、语义 verdict 归 agent)的字面落地**,决策见 [ADR-0011](../adr/0011-agent-supervised-verdict.md);无 supervisor 时退回结构性停机。core 零运行时依赖、同编 `wasm32`;8 单测 + 6 集成测试绿。
...
> **✅ 已落地(证据 [04 #26](04-spike-evidence.md))**:上表 reconcile loop + append-only DAG 已由 `crates/core/src/agent/orchestrator.rs` 的 `MiniOrchestrator`(单线程、运行时无关、零 tokio/std::time)驱动闭环 —— `loop { ready() → 经注入 StepRunner 端口派发就绪步 → mark Done/Failed → reconcile 边界持久化 } until is_complete()||has_failure()`。**level-triggered 幂等**:mid-reconcile 崩溃恢复重跑 `ready()`,已 Done 步不重派(对齐 [ADR-0005](../adr/0005-round-boundary-checkpoint.md) 与 `agent_recovery` 的"已完成工具不重调");失败短路保留 last-good、重复 reconcile 幂等 no-op。这正是 Argo `operate()` + K8s level-triggered([08](08-control-data-plane-separation.md))在 agent 编排上的落地。并行 fan-out `ready()` 可返回多 id:轻量版顺序派发(已证),Kameo 并行监督标注 future —— **parity 点:同一份 reconcile 逻辑即端上轻量核**。core 4 测试绿、同编 `wasm32`。
