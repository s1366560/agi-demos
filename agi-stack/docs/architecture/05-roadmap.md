...
| **1 · 抽取可移植核心** | domain(已纯净)+ application 纯逻辑迁入核心包;ports 落为跨平台 trait | 🚧 进行中(生产级 Cargo workspace 已落地于 `agi-stack/`:`core`+`plugin-host`+`adapters-{mem,device,wasmtime,http-llm,postgres}`+`bindings-{wasm,uniffi}`+`apps/{server,gateway,desktop,bench}`,`cargo test --workspace` **87 绿**、服务器全端点 curl 验证;完整 86K LOC Python→Rust 绞杀迁移为多季度工程,标注 future) |
...
| **5 · 逐平台上线** | 先服务器对齐现有功能(绞杀替换旧 Python),再 PC → 移动端 → web-WASM | 🚧 进行中(**构建矩阵已落地** + **生产绞杀迁移已启动**):[`Makefile`](../../Makefile) 一键构建 server/web/桌面/Android/iOS 五端产物 + 出厂矩阵 [09](09-shipping-matrix.md)([04 #24](04-spike-evidence.md));**Python 逐能力绞杀替换第一垂直落地** —— P1 记忆/情节/召回经 Rust 服务器读写 Python 同一套 Postgres(零数据迁移)+ 反向代理网关按能力分流 + 契约字节兼容,端到端跑通、可灰度可回滚(见 [04 #25](04-spike-evidence.md)、[10-production-migration](10-production-migration.md));P2–P8 余下 ~580 端点为多季度工程,逐波次标注 future |
...
| 11 | **可插拔 Harness**:Agent 执行循环能否做成可第三方替换的 `RuntimeHarness`(embedded vs CLI-backend),`auto` 选择 + 回退,且 runtime 按 `(provider,model)` scoped? | ✅ 已落地(embedded 侧)——`RuntimeHarness` trait + `HarnessRegistry::select`(`auto` 优先级 + policy pin + 回退),core 7 + `adapters-mem` 2 测试(见 [04 #14](04-spike-evidence.md)、[07 §4](07-plugin-runtime-architecture.md)、[ADR-0008](../adr/0008-agent-runtime-as-pluggable-harness.md));第二个非 embedded(CLI-backend)harness 待落地固化富契约 |
