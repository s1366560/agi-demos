...
| **F7 · 流式 LLM + WS 事件桥** | `adapters-http-llm` 扩流式 token(**决策 2:先 OpenAI/Anthropic 两家**,余下 future);WS ConnectionManager(1000+ 并发、广播、背压、重连、事件回放)。`axum::ws` + `tokio-tungstenite` | 🎯 future(P3·P4·P6)——**非流式嵌入半已对活 Ollama `/v1/embeddings` 实测**([04 #38](04-spike-evidence.md));流式 chat token + WS 桥待真 chat provider |
