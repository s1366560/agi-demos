//! Embedded SQLite [`MemoryRepository`] — the on-device storage path. The server
//! uses Postgres/pgvector behind the same port; only the wiring differs.
//!
//! Note: rusqlite is synchronous; the work per call is tiny so we run it inline
//! in the async methods. A production server adapter would offload to a blocking
//! pool. This adapter also **overrides** `search_by_project` to push the filter
//! into SQL (vs the in-core substring default) — exercising the port's override
//! path.
...
/// SQLite-backed memory repository. Bundled SQLite is compiled from source
/// (`features = ["bundled"]`), so this cross-compiles cleanly to iOS/Android.
...
pub(crate) fn to_storage<E: std::fmt::Display>(e: E) -> CoreError {
...
impl MemoryRepository for SqliteMemoryRepository {
    ...
    async fn search_by_project(
        ...
        let mut stmt = conn
            ...
            .map_err(to_storage)?;
        let rows = stmt
            .query_map(params![project_id, like, limit as i64], row_to_memory)
            .map_err(to_storage)?;
        let mut out = Vec::new();
        for r in rows {
            out.push(r.map_err(to_storage)?);
