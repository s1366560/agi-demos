...
//!
//! Mirrors OpenClaw's package contract: a package declares its resources under a
//! `package.json` field (`docs/plugins/manifest.md`). Here a small JSON shape
//! carries the same idea so the loader can discover what a plugin contributes
...
pub struct ToolDecl {
    ...
    #[serde(default)]
    pub trust: String,
    /// Optional inline WAT for wasm tools (a real package would ship a `.wasm`
    /// artifact reference instead).
    #[serde(default)]
