...
async def create_native_graph_adapter(
    ...
    llm_client = create_langchain_llm(tenant_id)
...
async def _create_embedding_service(settings, tenant_id: Optional[str] = None):
    ...
    if provider == "qwen":
        ...
        config = QwenEmbedderConfig(
            embedding_model=settings.qwen_embedding_model,
            embedding_dim=EMBEDDING_DIMS["qwen"],
            api_key=settings.qwen_api_key,
        )
        embedder = QwenEmbedder(config=config)
    elif provider in ("zai", "zhipu"):
        # Use ZAI/ZhipuAI native embedder
        from src.infrastructure.llm.zai import ZAISimpleEmbedder, ZAISimpleEmbedderConfig

        # Get API key (zai takes priority, fallback to zhipu)
        ...
        config = ZAISimpleEmbedderConfig(
            embedding_model=embedding_model or "embedding-3",
            embedding_dim=embedding_dim,
            api_key=api_key,
            base_url=base_url,
        )
        embedder = ZAISimpleEmbedder(config=config)
    else:
        # Default to Qwen for other providers (gemini, openai)
        config = QwenEmbedderConfig(
    ...
    logger.info(f"EmbeddingService created with provider: {provider}, dim: {embedding_dim}")
...
def create_langchain_llm(tenant_id: Optional[str] = None) -> LLMClient:
    ...
    Note: The function name is kept for backward compatibility but now
    returns our domain LLMClient instead of LangChain's BaseChatModel.
