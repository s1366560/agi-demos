...
class ChannelService:
    ...
    async def broadcast(
        self, to: str, content: MessageContent, channels: list[str] | None = None
    ) -> dict[str, str | None]:
    ...
    def _handle_message(self, message: Message) -> None:
        ...
        logger.debug(
            "Received message from channel: channel=%s has_text=%s text_length=%s",
            message.channel,
            bool(text),
            len(text),
        )
    ...
    async def get_user_info(self, channel_id: str, user_id: str) -> SenderInfo | None:
