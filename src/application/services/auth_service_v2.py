...
class AuthService:
    ...
    @staticmethod
    def verify_api_key_hash(key: str, hashed_key: str) -> bool:
    ...
    async def verify_api_key(self, api_key: str) -> Optional[APIKey]:
        ...
        if not api_key.startswith("ms_sk_"):
            raise ValueError("Invalid API key format. API keys should start with 'ms_sk_'")
        ...
        await self._api_key_repo.update_last_used(stored_key.id, datetime.now(timezone.utc))
