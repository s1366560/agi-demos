...
class TaskSessionCreationReceiptModel(Base):
    """Platform saga journal for one Core-owned atomic task-session creation."""
    ...
    initial_message_id: Mapped[str | None] = mapped_column(String, nullable=True)
    core_receipt_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    status: Mapped[str] = mapped_column(
        String(32),
        default="pending",
    ...
    __table_args__ = (
        ...
        Index(
            "ix_task_session_receipts_status_updated",
            "tenant_id",
            "project_id",
            "status",
            "updated_at",
        ...
        CheckConstraint(
            "status IN ('pending', 'core_committed', 'completed', 'retryable_error')",
            name="ck_task_session_receipts_status",
        ),
    )
...
class PlatformPluginApplyStateEventModel(IdGeneratorMixin, Base):
    """Append-only ACK/NACK receipt used to audit rollback drills."""

    __tablename__ = "platform_plugin_apply_state_events"
...
class PlatformPluginPackageModel(Base):
    ...
    artifact_registry: Mapped[str] = mapped_column(String(512), nullable=False)
    artifact_repository: Mapped[str] = mapped_column(String(255), nullable=False)
    oci_manifest_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    install_status: Mapped[str] = mapped_column(String(32), nullable=False, default="verified")
    manifest: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False, default=dict)
