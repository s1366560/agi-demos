...
class SqlWorkspaceRepository(BaseRepository[Workspace, WorkspaceModel], WorkspaceRepository):
    ...
    @override
    async def save(self, workspace: Workspace) -> Workspace:  # pyright: ignore[reportIncompatibleMethodOverride]
        return await super().save(workspace)

    @override
    async def find_by_id(self, workspace_id: str) -> Workspace | None:  # pyright: ignore[reportIncompatibleMethodOverride]
        return await super().find_by_id(workspace_id)

    @override
    async def delete(self, workspace_id: str) -> bool:  # pyright: ignore[reportIncompatibleMethodOverride]
        return await super().delete(workspace_id)
    ...
    async def find_by_project(
        ...
        query = (
            ...
            .order_by(WorkspaceModel.created_at.desc(), WorkspaceModel.id.asc())
            .offset(offset)
            .limit(limit)
        )
    ...
    @override
    def _to_domain(self, db_model: WorkspaceModel | None) -> Workspace | None:
        if db_model is None:
            return None
        ...
        return Workspace(
            ...
            office_status=db_model.office_status,
            hex_layout_config=db_model.hex_layout_config_json or {},
            default_blocking_categories=list(db_model.default_blocking_categories_json or ()),
