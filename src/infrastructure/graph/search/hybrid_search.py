...
class HybridSearch:
    ...
    def _escape_fulltext_query(self, query: str) -> str:
        """
        Escape special characters and bound term count for Neo4j fulltext search.
        ...
        return self._limit_fulltext_query_terms(escaped)

    @staticmethod
    def _limit_fulltext_query_terms(query: str) -> str:
        """Cap Lucene clauses generated by long conversational queries."""
        terms = query.split()
        if not terms:
            return ""

        limited_terms: list[str] = []
        seen: set[str] = set()
        for term in terms:
            if term in seen:
                continue
            seen.add(term)
            limited_terms.append(term)
            if len(limited_terms) >= MAX_FULLTEXT_QUERY_TERMS:
                break
