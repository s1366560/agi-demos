...
class FailingOrchestrator:
    ...
    async def stop_desktop(self, sandbox_id: str) -> bool:
        raise RuntimeError(f"internal desktop stop secret for {sandbox_id}")
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_stop_desktop_sanitizes_internal_errors(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    monkeypatch.setattr(services_router, "assert_caller_owns_sandbox", _allow_sandbox_access)
    ...
    with pytest.raises(HTTPException) as exc_info:
        await services_router.stop_desktop(
            sandbox_id="sandbox-secret",
            current_user=SimpleNamespace(id="user-1", is_superuser=True),
            adapter=SimpleNamespace(),
            orchestrator=FailingOrchestrator(),
    ...
    assert exc_info.value.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    assert exc_info.value.detail == "Failed to stop desktop"
    assert "internal" not in exc_info.value.detail
    assert "sandbox-secret" not in exc_info.value.detail
    target_records = [
        record
    ...
    assert "internal desktop stop secret" not in message
