...
@pytest.mark.unit
def test_stop_project_desktop_error_log_omits_ids_and_exception_text(
    sandbox_http_client: TestClient,
    caplog: pytest.LogCaptureFixture,
) -> None:
    lifecycle_service = AsyncMock()
    lifecycle_service.get_project_sandbox = AsyncMock(return_value=_sandbox_info())
    orchestrator = SimpleNamespace(
        stop_desktop=AsyncMock(side_effect=RuntimeError("desktop stop secret for proj-1"))
    )
    sandbox_http_client.app.dependency_overrides[router_mod.get_lifecycle_service] = (
        lambda: lifecycle_service
    )
    ...
    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    assert response.json()["detail"] == "Failed to stop desktop"
    assert "proj-1" not in response.text
    assert "desktop stop secret" not in response.text
    assert "Failed to stop desktop" in caplog.text
    assert "has_project_id=True" in caplog.text
    assert "error_type=RuntimeError" in caplog.text
    assert "proj-1" not in caplog.text
    assert "desktop stop secret" not in caplog.text
...
@pytest.mark.unit
def test_start_project_terminal_error_log_omits_ids_and_exception_text(
    sandbox_http_client: TestClient,
    caplog: pytest.LogCaptureFixture,
) -> None:
    lifecycle_service = AsyncMock()
    lifecycle_service.ensure_sandbox_running = AsyncMock(return_value=_sandbox_info())
    orchestrator = SimpleNamespace(
        start_terminal=AsyncMock(side_effect=RuntimeError("terminal secret for proj-1"))
    )
    sandbox_http_client.app.dependency_overrides[router_mod.get_lifecycle_service] = (
        lambda: lifecycle_service
    )
    ...
    response = sandbox_http_client.post("/api/v1/projects/proj-1/sandbox/terminal")

    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    assert response.json()["detail"] == "Failed to start terminal"
    assert "proj-1" not in response.text
    assert "terminal secret" not in response.text
    assert "Failed to start terminal" in caplog.text
    assert "has_project_id=True" in caplog.text
    assert "error_type=RuntimeError" in caplog.text
    assert "proj-1" not in caplog.text
    assert "terminal secret" not in caplog.text


@pytest.mark.unit
def test_stop_project_terminal_error_log_omits_ids_and_exception_text(
    sandbox_http_client: TestClient,
    caplog: pytest.LogCaptureFixture,
) -> None:
    lifecycle_service = AsyncMock()
    lifecycle_service.get_project_sandbox = AsyncMock(return_value=_sandbox_info())
    orchestrator = SimpleNamespace(
        stop_terminal=AsyncMock(side_effect=RuntimeError("terminal stop secret for proj-1"))
    )
    sandbox_http_client.app.dependency_overrides[router_mod.get_lifecycle_service] = (
        lambda: lifecycle_service
    )
    ...
    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    assert response.json()["detail"] == "Failed to stop terminal"
    assert "proj-1" not in response.text
    assert "terminal stop secret" not in response.text
    assert "Failed to stop terminal" in caplog.text
    assert "has_project_id=True" in caplog.text
    assert "error_type=RuntimeError" in caplog.text
    assert "proj-1" not in caplog.text
    assert "terminal stop secret" not in caplog.text
