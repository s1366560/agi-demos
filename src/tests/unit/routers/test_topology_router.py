...
@pytest.fixture
def topology_client(mock_topology_service: AsyncMock, monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    monkeypatch.setattr(
        "src.infrastructure.adapters.primary.web.routers.topology.publish_workspace_event",
        publish_mock,
    )
...
class TestTopologyRouter:
    ...
    def test_update_edge_workspace_validation_failure(
        ...
        mock_topology_service.update_edge.side_effect = ValueError("Endpoints must be in same workspace")
