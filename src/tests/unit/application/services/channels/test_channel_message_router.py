...
async def test_invoke_agent_streams_via_card_when_adapter_supports_it() -> None:
    """When adapter supports streaming cards, router should send + patch cards."""
    router = ChannelMessageRouter()
    router._send_response = AsyncMock()
    router._send_error_feedback = AsyncMock()
    router._broadcast_workspace_event = AsyncMock()
    ...
    # Build a fake adapter that supports streaming
    ...
    # Mock _get_streaming_adapter to return the fake adapter
    ...
    # Should have sent initial streaming card
    fake_adapter.send_streaming_card.assert_awaited_once()

    # Should NOT have called _send_response (streaming path handles it)
    router._send_response.assert_not_awaited()

    # Should have recorded streaming outbox
...
async def test_invoke_agent_falls_back_when_patch_card_fails() -> None:
    """When patch_card fails, router should fall through to _send_response."""
    router = ChannelMessageRouter()
    router._send_response = AsyncMock()
    router._send_error_feedback = AsyncMock()
    router._broadcast_workspace_event = AsyncMock()
    ...
    # Build a fake adapter where patch_card fails
    fake_adapter = MagicMock()
    fake_adapter.send_streaming_card = AsyncMock(return_value="om_stream_1")
    fake_adapter.patch_card = AsyncMock(return_value=False)  # patch fails
    fake_adapter._build_streaming_card = MagicMock(return_value='{"elements":[]}')
    ...
    # patch_card failed, so should fall through to _send_response
    router._send_response.assert_awaited_once_with(message, "conv-1", "Hello world")
    # Should NOT have recorded streaming outbox (patch failed)
