...
class TestSandboxMCPRuleset:
    ...
    def test_sandbox_read_tools_allowed_by_default(self):
        ...
        rule = evaluate_rules("read", "sandbox_abc123def_file_read", rules)
        assert rule.action == PermissionAction.ALLOW

        rule = evaluate_rules("read", "sandbox_xyz789_list_files", rules)
        assert rule.action == PermissionAction.ALLOW
    ...
    def test_sandbox_write_tools_require_ask(self):
        ...
        rule = evaluate_rules("write", "sandbox_abc123def_file_write", rules)
        assert rule.action == PermissionAction.ASK

        rule = evaluate_rules("write", "sandbox_xyz789_create_file", rules)
        assert rule.action == PermissionAction.ASK

        rule = evaluate_rules("write", "sandbox_xyz789_edit_file", rules)
        assert rule.action == PermissionAction.ASK
    ...
    def test_sandbox_bash_tools_require_ask(self):
        ...
        rule = evaluate_rules("bash", "sandbox_abc123def_bash", rules)
        assert rule.action == PermissionAction.ASK

        rule = evaluate_rules("bash", "sandbox_xyz789_execute", rules)
        assert rule.action == PermissionAction.ASK

    def test_sandbox_tools_match_any_sandbox_id(self):
        """Test that rules match regardless of sandbox ID."""
        rules = sandbox_mcp_ruleset()

        # Different sandbox IDs should still match
        for sandbox_id in ["abc123", "xyz789", "test123", "prod456"]:
            tool_name = f"sandbox_{sandbox_id}_file_read"
...
class TestSandboxMCPRulesetIntegration:
    ...
    async def test_sandbox_read_tool_granted(self):
        ...
        result = await manager.ask(
            permission="read",
            patterns=["sandbox_abc123def_file_read"],
    ...
    async def test_sandbox_write_tool_requires_permission(self):
        ...
        rule = manager.evaluate("write", "sandbox_abc123def_file_write")
        assert rule.action == PermissionAction.ASK
    ...
    async def test_sandbox_bash_tool_denied_in_plan_mode(self):
        ...
        rule = manager.evaluate("bash", "sandbox_abc123def_bash")
    ...
    async def test_sandbox_tools_denied_in_explore_mode(self):
        ...
        rule = manager.evaluate("read", "sandbox_abc123def_file_read")
        assert rule.action == PermissionAction.DENY
...
class TestSandboxMCPDisabledTools:
    ...
    def test_sandbox_tools_disabled_in_plan_mode(self):
        ...
        tools = [
            "sandbox_abc123def_file_read",
            "sandbox_abc123def_file_write",
            "sandbox_abc123def_bash",
        ]
        ...
        write_rule = evaluate_rules("write", "sandbox_abc123def_file_write", plan_rules)
        bash_rule = evaluate_rules("bash", "sandbox_abc123def_bash", plan_rules)
