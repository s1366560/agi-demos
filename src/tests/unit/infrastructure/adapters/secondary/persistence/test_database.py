...
async def test_initialize_database_repairs_agent_events_schema_before_stamping() -> None:
    ...
    with (
        ...
        patch.object(database, "update_agent_events_schema", new=AsyncMock(side_effect=_record_schema_update)),
...
async def test_update_agent_events_schema_widens_correlation_id_and_adds_index() -> None:
    ...
    executed_sql = [
        " ".join(str(await_call.args[0]).split())
        for await_call in conn.execute.await_args_list
    ]
