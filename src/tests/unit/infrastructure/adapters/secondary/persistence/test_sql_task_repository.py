...
@pytest.fixture
async def v2_task_repo(v2_db_session: AsyncSession) -> SqlTaskRepository:
    """Create a V2 task repository for testing."""
    return SqlTaskRepository(v2_db_session)


class TestSqlTaskRepositoryCreate:
    """Tests for creating tasks."""

    @pytest.mark.asyncio
    async def test_save_new_task(self, v2_task_repo: SqlTaskRepository):
...
class TestSqlTaskRepositoryUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_existing_task(self, v2_task_repo: SqlTaskRepository):
...
class TestSqlTaskRepositoryFind:
    """Tests for finding tasks."""

    @pytest.mark.asyncio
    async def test_find_by_id_existing(self, v2_task_repo: SqlTaskRepository):
    ...
    @pytest.mark.asyncio
    async def test_find_by_id_not_found(self, v2_task_repo: SqlTaskRepository):
        """Test finding a non-existent task returns None."""
        retrieved = await v2_task_repo.find_by_id("non-existent")
        assert retrieved is None

    @pytest.mark.asyncio
    async def test_find_by_group(self, v2_task_repo: SqlTaskRepository):
...
class TestSqlTaskRepositoryDelete:
    ...
    @pytest.mark.asyncio
    async def test_delete_existing_task(self, v2_task_repo: SqlTaskRepository):
...
class TestSqlTaskRepositoryToDomain:
    """Tests for _to_domain conversion."""

    def test_to_domain_with_none(self, v2_task_repo: SqlTaskRepository):
        """Test that _to_domain returns None for None input."""
        result = v2_task_repo._to_domain(None)
        assert result is None


class TestSqlTaskRepositoryToDb:
    """Tests for _to_db conversion."""

    def test_to_db_creates_db_model(self, v2_task_repo: SqlTaskRepository):
