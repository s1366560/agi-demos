...
class MultiHITLTest:
    ...
    async def test_processor_handler_lifecycle(self):
        ...
        print(
            f"  [Cycle 1] Handler created, has preinjected: {handler1._preinjected_response is not None}"
        )
        ...
        if handler2._preinjected_response is None:
            ...
            print(
                f"  [Cycle 2] Preinjected request_id: {handler2._preinjected_response.get('request_id')}"
            )
