...
- Models：一级对象为 Provider。Overview 汇总连接健康、已启用模型和工作区路由；Connection 管理 Auth method、Credential reference、Base URL、后端显式声明的结构化请求策略与 Test connection；Models 管理发现、搜索、启停和手动 Model ID；Routing 管理 Default / Fast / Coding / Vision 与 Fallback chain；Usage 显示 Success、Latency、Spend 和连接事件。
...
- Connection 以 capability descriptor 呈现 OAuth、API Key、Environment secret、No authentication；支持项可选择，不可用项保留可见但禁用并解释原因。能力列表加载中、加载失败、缺少目标 Provider 或明确返回空认证方式时，认证选择和保存必须 fail closed。OAuth 没有后端 start/callback 能力时不得打开窗口或模拟成功；Environment secret 只输入后端实际检测或允许的变量名，只能与供应商官方 HTTPS Origin 配对，并显示由 `probed` 证据驱动的 Available / Not available / Check required；自定义 Endpoint 改用加密 API Key。切换认证方式清空互斥字段；修改 Endpoint 后不得复用旧凭据；清空 Endpoint 发送显式 `null`。默认隐藏 API mode 与 Timeout 等结构化高级字段；自由 Headers、自由 Provider JSON 和明文辅助凭据不进入表单。保存前必须 Test connection，若 capability 声明不支持探测则显示“配置校验通过”，不能显示连接成功。
