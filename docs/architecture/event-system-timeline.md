...
| 指标 | 数值 | 说明 |
|------|------|------|
| 事件类型总数 | **165 种** | AgentEventType 枚举 |
| 事件类别 | **5 个** | AGENT, HITL, SANDBOX, SYSTEM, MESSAGE |
| 前端已处理 | 见下文说明 | 处理逻辑分散在多个文件，不在此固定一个静态计数 |
| 内部事件 | **2 种** | COMPACT_NEEDED, RETRY (不暴露给前端) |
| Delta 事件 (不持久化) | **6 种** | 流式片段，仅实时传输 |
| Terminal 事件 | **3 种** | COMPLETE, ERROR, CANCELLED |
| HITL 交互事件 | **6 种** | 需要用户响应 |

> 前端处理分散在 `web/src/services/agentService.ts`、`messageRouter.ts`、`sseEventAdapter.ts`
> 以及 `web/src/stores/agent/*` 等多个文件中，新增/修改事件时应以源码检索为准，不要依赖本文
> 档中的静态"已处理计数"。参见 `docs/agent-event-types.md` 的 Frontend Handling 一节。
...
> 类别计数依据 `src/domain/events/types.py` 中的 `EVENT_CATEGORIES` 字典：未在字典中显式
> 登记的枚举值会被 `get_event_category()` 默认归类为 `AGENT`，因此运行时的实际 AGENT 数
> 量大于字典中显式映射的数量。
