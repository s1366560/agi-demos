...
use bcs_db_api::{
    DbPlugin, DbRow, DbSqlFlavor, DbStatement, DbStatementBuilder, DbValue as Value, db_get_column,
    db_get_column_opt,
};
use bcs_service_api::{
...
impl PersistentBotRepo {
    ...
    pub fn with_plugins(cache: Arc<dyn CachePlugin>, db: Arc<dyn DbPlugin>) -> Self {
    ...
    pub fn with_plugins_flavor(
        ...
        Self::with_plugins_flavor_and_cache_key_prefix(cache, db, flavor, DEFAULT_CACHE_KEY_PREFIX)
    }
    ...
    pub fn new(cache: Arc<dyn CachePlugin>, db: Arc<dyn DbPlugin>, _legacy_db: String) -> Self {
    ...
    fn status_cache_key_with_prefix(cache_key_prefix: &str, bot_uuid: &str) -> String {
        format!(
            "{}{}{}",
            cache_key_prefix, STATUS_CACHE_KEY_NAMESPACE, bot_uuid
        )
    }

    async fn db_query(&self, statement: DbStatement) -> bcs_db_api::DbResult<Vec<DbRow>> {
        self.db.query(statement).await
    }

    async fn db_execute_affected(&self, statement: DbStatement) -> bcs_db_api::DbResult<u64> {
        self.db
            .execute(statement)
            .await
        ...
        let affected = if exists {
            ...
            if let Some(token) = session_token {
                let statement = DbStatementBuilder::new(self.flavor)
                    ...
                    .bind(bot_info_json.as_str())
                    .push_static(", session_token = ")
                    .bind(token)
                    .push_static(", visibility = ")
                    .bind(visibility)
                    .push_static(", agent_code = ")
                    .bind(agent_code_value.clone())
                    .push_static(", updated_at = CURRENT_TIMESTAMP WHERE bot_uuid = ")
                    .bind(bot_uuid)
                    .push_static(" AND env = ")
                    .bind(env.as_str())
                    .build();
                self.db_execute_affected(statement).await
            } else {
                let statement = DbStatementBuilder::new(self.flavor)
                    .push_static("UPDATE bcs_bots SET name = ")
                    .bind(name)
            ...
            let statement = DbStatementBuilder::new(self.flavor)
                .push_static(
                    "INSERT INTO bcs_bots \
                     (bot_uuid, name, bot_info, session_token, created_by, visibility, status, \
                      actor_kind, agent_code, is_deleted, env, registered_at, updated_at) VALUES (",
                )
                .bind(bot_uuid)
                .push_static(", ")
                .bind(name)
                ...
                .push_static(", ")
                .bind(env.as_str())
                .push_static(", CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)")
                .build();
            self.db_execute_affected(statement).await
        }
        .map_err(|e| {
    ...
    > {
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static(
                "SELECT name, bot_info, visibility, status, actor_kind, env, created_by, agent_code \
                 FROM bcs_bots WHERE bot_uuid = ",
            )
            .bind(bot_uuid)
            .push_static(" AND env = ")
            .bind(env.as_str());
        let statement = if include_deleted {
            statement
        } else {
            statement.push_static(" AND is_deleted = ").bind(false)
        }
        .build();

        let rows = self.db_query(statement).await.map_err(|error| {
        ...
        if let Some(row) = rows.first() {
            ...
            let col_agent_code: Option<String> =
                db_get_column_opt(row, "agent_code").ok().flatten();
    ...
    async fn read_bot_info_json(&self, bot_uuid: &str) -> Option<serde_json::Value> {
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("SELECT bot_info FROM bcs_bots WHERE bot_uuid = ")
            .bind(bot_uuid)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(" AND is_deleted = ")
            .bind(false)
            .push_static(" LIMIT 1")
            .build();
        let rows = self.db_query(statement).await.ok()?;
        let row = rows.first()?;
    ...
    async fn exists_in_db(&self, bot_uuid: &str) -> bool {
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("SELECT 1 FROM bcs_bots WHERE bot_uuid = ")
            .bind(bot_uuid)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(" LIMIT 1")
            .build();
        self.db_query(statement)
            .await
            .map(|r| !r.is_empty())
            .unwrap_or(false)
    }
    ...
    async fn save_token_to_db(&self, bot_uuid: &str, token: &str) -> ServiceResult<()> {
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("UPDATE bcs_bots SET session_token = ")
            .bind(token)
        ...
        let affected = self.db_execute_affected(statement).await.map_err(|e| {
    ...
    async fn update_created_by_in_db(
        ...
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("UPDATE bcs_bots SET created_by = ")
            .bind(created_by)
            .push_static(" WHERE bot_uuid = ")
            .bind(bot_uuid)
            .push_static(" AND env = ")
        ...
        let statement = if overwrite {
            ...
            statement.push_static(" AND created_by IS NULL")
        }
        .build();
    ...
    async fn list_bots_by_creator_from_db(
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static(
                "SELECT bot_uuid, name, bot_info, visibility, status, actor_kind, env, created_by \
                 FROM bcs_bots WHERE created_by = ",
            )
            .bind(created_by)
            .push_static(" AND env = ")
            .bind(env)
    ...
    async fn load_token_from_db(&self, bot_uuid: &str) -> Option<String> {
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("SELECT session_token FROM bcs_bots WHERE bot_uuid = ")
            .bind(bot_uuid)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(" AND is_deleted = ")
    ...
    async fn find_bot_by_token_in_db(&self, token: &str) -> Option<String> {
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("SELECT bot_uuid FROM bcs_bots WHERE session_token = ")
            .bind(token)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(" AND is_deleted = ")
        ...
        let rows = match self.db_query(statement).await {
            Ok(rows) => rows,
            Err(e) => {
    ...
    async fn find_bot_by_agent_code_in_db(&self, agent_code: &str) -> Option<String> {
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("SELECT bot_uuid FROM bcs_bots WHERE agent_code = ")
            .bind(agent_code)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(" AND is_deleted = ")
    ...
    async fn soft_delete_in_db(&self, bot_uuid: &str) -> bool {
        let env = resolve_env();
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("UPDATE bcs_bots SET is_deleted = ")
            .bind(true)
            .push_static(", updated_at = CURRENT_TIMESTAMP WHERE bot_uuid = ")
            .bind(bot_uuid)
            .push_static(" AND env = ")
        ...
        self.db_execute_affected(statement)
            .await
            .map(|n| n > 0)
            .unwrap_or(false)
    }
    ...
    async fn list_bots_by_name_and_cooperatable_with_impl(
        ...
        let limit = match i64::try_from(limit) {
            Ok(limit) => limit,
            Err(error) => {
                warn!(error = %error, "bot search limit is out of range");
                return (Vec::new(), 0);
            }
        };
        let offset = match i64::try_from(offset) {
            Ok(offset) => offset,
            Err(error) => {
                warn!(error = %error, "bot search offset is out of range");
                return (Vec::new(), 0);
            }
        };

        let page_statement = DbStatementBuilder::new(self.flavor)
            .push_static(
                "WITH friend_uuids AS (\
                    SELECT right_bot AS bot_uuid FROM bcs_friendships WHERE left_bot = ",
            )
            .bind(bot_uuid)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(
                " UNION SELECT left_bot AS bot_uuid FROM bcs_friendships WHERE right_bot = ",
            )
            .bind(bot_uuid)
            .push_static(" AND env = ")
            .bind(env.as_str())
            .push_static(
                ") SELECT b.bot_uuid, b.name, b.bot_info, b.visibility, b.env, b.created_by, \
                    b.actor_kind, b.status, f.bot_uuid IS NOT NULL as is_friend \
                 FROM bcs_bots b LEFT JOIN friend_uuids f ON b.bot_uuid = f.bot_uuid \
                 WHERE b.env = ",
            )
            .bind(env.as_str())
            .push_static(" AND b.is_deleted = ")
            .bind(false)
            .push_static(" AND b.name LIKE ")
            .bind(like.as_str())
            .push_static(" AND b.bot_uuid != ")
            .bind(bot_uuid)
            .push_static(
                " AND (b.actor_kind IS NULL OR b.actor_kind != 'human') \
                 AND CASE WHEN ",
            )
            .bind(cooperatable_only)
            .push_static(
                " THEN b.visibility = 'public' OR f.bot_uuid IS NOT NULL \
                 ELSE b.visibility IN ('public', 'protected') END \
        ...
        let page_rows = match self.db_query(page_statement).await {
            Ok(r) => r,
            Err(e) => {
        ...
        let count_statement = DbStatementBuilder::new(self.flavor)
            ...
            .bind(env.as_str())
            .push_static(
                ") SELECT count(*) AS total FROM bcs_bots b \
                 LEFT JOIN friend_uuids f ON b.bot_uuid = f.bot_uuid WHERE b.env = ",
            )
            .bind(env.as_str())
            .push_static(" AND b.is_deleted = ")
            .bind(false)
        ...
        let total: usize = match self.db_query(count_statement).await {
            Ok(rows) => rows
                .first()
        ...
        for row in &page_rows {
            ...
            let bot_info: BotInfo = db_get_column_opt::<String>(row, "bot_info")
                ...
                .and_then(|s| serde_json::from_str(&s).ok())
                .unwrap_or_default();
            let is_friend = row.get_bool("is_friend").ok().flatten().unwrap_or(false);
...
impl BotMetricsSnapshotPort for PersistentBotRepo {
    async fn bot_counts(&self) -> ServiceResult<Vec<BotMetricCount>> {
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static(
                "SELECT actor_kind, status, visibility, COUNT(*) AS bot_count \
                 FROM ( \
                     SELECT \
                         ...
                         CASE \
                             ...
                             ELSE 'other' \
                         END AS visibility \
                     FROM bcs_bots WHERE env = ",
            )
            .bind(env.as_str())
            .push_static(" AND is_deleted = ")
            .bind(false)
            .push_static(" ) metric_bots GROUP BY actor_kind, status, visibility")
            .build();
        let rows = self.db_query(statement).await.map_err(|e| {
...
impl BotRepoPort for PersistentBotRepo {
    ...
    async fn update_visibility(&self, bot_id: &str, visibility: &str) -> ServiceResult<()> {
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("UPDATE bcs_bots SET visibility = ")
            ...
            .bind(env.as_str())
            .build();
        self.db_execute_affected(statement).await.map_err(|e| {
    ...
    async fn update_actor_status(
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static("UPDATE bcs_bots SET status = ")
            .bind(status_str)
            .push_static(" WHERE bot_uuid = ")
            .bind(bot_id)
            .push_static(" AND env = ")
    ...
    async fn ensure_human_actor(
        ...
        if self.exists_in_db(&bot_uuid).await {
            ...
            if needs_backfill {
                let statement = DbStatementBuilder::new(self.flavor)
                    .push_static("UPDATE bcs_bots SET bot_info = ")
                    .bind(merged_str.as_str())
                    .push_static(" WHERE bot_uuid = ")
        ...
        .push_static(
            " INTO bcs_bots \
             (bot_uuid, actor_kind, name, bot_info, session_token, created_by, visibility, status, \
              env, registered_at, updated_at) VALUES (",
        )
        .bind(bot_uuid.as_str())
        .push_static(", ")
        .bind(actor_kind)
        ...
        let statement = match self.flavor {
            DbSqlFlavor::Postgres => {
                statement.push_static(" ON CONFLICT(bot_uuid, env) DO NOTHING")
            }
            DbSqlFlavor::Mysql | DbSqlFlavor::Sqlite => statement,
        }
        .build();
        let affected = self.db_execute_affected(statement).await.map_err(|e| {
            warn!(
    ...
    async fn list_legacy_bots_for_owner(
        ...
        let statement = DbStatementBuilder::new(self.flavor)
            .push_static(
                "SELECT bot_uuid, actor_kind, name, bot_info, session_token, \
                        visibility, status, created_by FROM bcs_bots WHERE env = ",
            )
            .bind(env)
            .push_static(" AND actor_kind = 'bot' AND (created_by = ")
            .bind(staff_no)
            .push_static(" OR (created_by IS NULL AND bot_uuid LIKE ")
    ...
    async fn find_bot_by_agent_code(&self, agent_code: &str) -> Option<String> {
        ...
        let memory_candidate = {
            let bots = self.bots.read().await;
            bots.iter()
                .find(|(_, bot)| bot.capabilities.agent_code.as_deref() == Some(agent_code))
                .map(|(bot_id, _)| bot_id.clone())
        };
        if let Some(bot_id) = memory_candidate {
            if self.get(&bot_id).await.is_some() {
...
fn push_control_plane_projection(
    statement: DbStatementBuilder,
    flavor: DbSqlFlavor,
    aliased: bool,
) -> DbStatementBuilder {
    match (flavor, aliased) {
        (DbSqlFlavor::Mysql, false) => statement.push_static(
            "SELECT bot_uuid, name, bot_info, visibility, status, actor_kind, env, created_by, \
             agent_code, UNIX_TIMESTAMP(gmt_create) * 1000 AS gmt_create_ms, \
             UNIX_TIMESTAMP(gmt_modified) * 1000 AS gmt_modified_ms",
        ),
        (DbSqlFlavor::Sqlite, false) => statement.push_static(
            "SELECT bot_uuid, name, bot_info, visibility, status, actor_kind, env, created_by, \
             agent_code, CAST(strftime('%s', gmt_create) AS INTEGER) * 1000 AS gmt_create_ms, \
             CAST(strftime('%s', gmt_modified) AS INTEGER) * 1000 AS gmt_modified_ms",
        ),
        (DbSqlFlavor::Postgres, false) => statement.push_static(
            "SELECT bot_uuid, name, bot_info, visibility, status, actor_kind, env, created_by, \
             agent_code, CAST(EXTRACT(EPOCH FROM gmt_create) AS BIGINT) * 1000 AS gmt_create_ms, \
             CAST(EXTRACT(EPOCH FROM gmt_modified) AS BIGINT) * 1000 AS gmt_modified_ms",
        ),
        (DbSqlFlavor::Mysql, true) => statement.push_static(
            "SELECT b.bot_uuid, b.name, b.bot_info, b.visibility, b.status, b.actor_kind, b.env, \
             b.created_by, b.agent_code, \
             UNIX_TIMESTAMP(b.gmt_create) * 1000 AS gmt_create_ms, \
             UNIX_TIMESTAMP(b.gmt_modified) * 1000 AS gmt_modified_ms",
        ),
        (DbSqlFlavor::Sqlite, true) => statement.push_static(
            "SELECT b.bot_uuid, b.name, b.bot_info, b.visibility, b.status, b.actor_kind, b.env, \
             b.created_by, b.agent_code, \
             CAST(strftime('%s', b.gmt_create) AS INTEGER) * 1000 AS gmt_create_ms, \
             CAST(strftime('%s', b.gmt_modified) AS INTEGER) * 1000 AS gmt_modified_ms",
        ),
        (DbSqlFlavor::Postgres, true) => statement.push_static(
            "SELECT b.bot_uuid, b.name, b.bot_info, b.visibility, b.status, b.actor_kind, b.env, \
             b.created_by, b.agent_code, \
             CAST(EXTRACT(EPOCH FROM b.gmt_create) AS BIGINT) * 1000 AS gmt_create_ms, \
             CAST(EXTRACT(EPOCH FROM b.gmt_modified) AS BIGINT) * 1000 AS gmt_modified_ms",
        ),
    }
}

fn friend_uuids_cte(flavor: DbSqlFlavor, friend_ids: &[String]) -> DbStatementBuilder {
    let mut statement = DbStatementBuilder::new(flavor).push_static("WITH friend_uuids AS (");
    if friend_ids.is_empty() {
        return statement.push_static("SELECT NULL AS bot_uuid WHERE 1 = 0) ");
    }
    for (index, friend_id) in friend_ids.iter().enumerate() {
        if index > 0 {
            statement = statement.push_static(" UNION ALL ");
        }
        statement = statement.push_static("SELECT ").bind(friend_id.as_str());
        if index == 0 {
            statement = statement.push_static(" AS bot_uuid");
        }
    }
...
impl BotControlPlaneRepoPort for PersistentBotRepo {
    async fn get_control_plane(
        ...
        let statement =
            push_control_plane_projection(DbStatementBuilder::new(self.flavor), self.flavor, false)
                .push_static(" FROM bcs_bots WHERE bot_uuid = ")
                .bind(bot_id)
                .push_static(" AND env = ")
                .bind(env)
                .push_static(" AND is_deleted = ")
    ...
    async fn list_control_plane_candidates(
        ...
        let limit = i64::try_from(query.limit).map_err(|error| {
            ServiceError::InternalError(format!("bot candidate limit is out of range: {error}"))
        })?;
        let offset = i64::try_from(query.offset).map_err(|error| {
            ServiceError::InternalError(format!("bot candidate offset is out of range: {error}"))
        })?;

        let page_statement = push_control_plane_projection(
            friend_uuids_cte(self.flavor, &friend_ids),
            self.flavor,
            true,
        )
        .push_static(
        ...
        .push_static(" AND b.is_deleted = ")
        .bind(false)
        .push_static(" AND COALESCE(b.actor_kind, 'bot') = 'bot' AND b.bot_uuid != ")
        .bind(query.acting_bot_id.as_str());
        let page_statement = match self.flavor {
            DbSqlFlavor::Mysql | DbSqlFlavor::Sqlite => page_statement
                .push_static(" AND INSTR(LOWER(b.name), ")
                .bind(name_filter.as_str())
                .push_static(") > 0"),
            DbSqlFlavor::Postgres => page_statement
                .push_static(" AND POSITION(")
                .bind(name_filter.as_str())
                .push_static(" IN LOWER(b.name)) > 0"),
        };
        let page_statement = match query.visibility {
            BotCandidateVisibility::Discovery => {
                page_statement.push_static(" AND b.visibility IN ('public', 'protected')")
            }
            BotCandidateVisibility::Collaboration => page_statement
                .push_static(" AND (b.visibility = 'public' OR f.bot_uuid IS NOT NULL)"),
        }
        .push_static(" ORDER BY b.gmt_create DESC, b.bot_uuid ASC LIMIT ")
        .bind(limit)
        .push_static(" OFFSET ")
        .bind(offset)
        .build();
        let rows = self
            .db_query(page_statement)
            .await
            .map_err(|error| ServiceError::InternalError(error.to_string()))?;
        ...
        let count_statement = friend_uuids_cte(self.flavor, &friend_ids)
            .push_static(
                "SELECT COUNT(*) AS total FROM bcs_bots b \
                 LEFT JOIN friend_uuids f ON b.bot_uuid = f.bot_uuid WHERE b.env = ",
            )
            ...
            .push_static(" AND COALESCE(b.actor_kind, 'bot') = 'bot' AND b.bot_uuid != ")
            .bind(query.acting_bot_id.as_str());
        let count_statement = match self.flavor {
            DbSqlFlavor::Mysql | DbSqlFlavor::Sqlite => count_statement
                .push_static(" AND INSTR(LOWER(b.name), ")
                .bind(name_filter.as_str())
                .push_static(") > 0"),
            DbSqlFlavor::Postgres => count_statement
                .push_static(" AND POSITION(")
                .bind(name_filter.as_str())
                .push_static(" IN LOWER(b.name)) > 0"),
        };
        let count_statement = match query.visibility {
            BotCandidateVisibility::Discovery => {
                count_statement.push_static(" AND b.visibility IN ('public', 'protected')")
            }
            BotCandidateVisibility::Collaboration => count_statement
                .push_static(" AND (b.visibility = 'public' OR f.bot_uuid IS NOT NULL)"),
        }
        .build();
        let count_rows = self
            .db_query(count_statement)
            .await
            .map_err(|error| ServiceError::InternalError(error.to_string()))?;
        let total = count_rows
            ...
            .map_err(|error| ServiceError::InternalError(error.to_string()))?
            .unwrap_or(0);
        let total = u64::try_from(total.max(0)).map_err(|error| {
            ServiceError::InternalError(format!("bot candidate total is out of range: {error}"))
        })?;
        Ok((records, total))
    }
    ...
    async fn list_control_plane_by_creator(
        ...
        let mut statement =
            push_control_plane_projection(DbStatementBuilder::new(self.flavor), self.flavor, false)
                .push_static(" FROM bcs_bots WHERE created_by = ")
                .bind(query.created_by.as_str())
                .push_static(" AND env = ")
                .bind(query.env.as_str())
                .push_static(" AND is_deleted = ")
                .bind(false);
        if let Some(kind) = query.kind {
            statement = statement
                .push_static(" AND COALESCE(actor_kind, 'bot') = ")
                .bind(match kind {
            ...
            .filter(|value| !value.is_empty())
        {
            ...
            statement = match self.flavor {
                DbSqlFlavor::Mysql | DbSqlFlavor::Sqlite => statement
                    .push_static(" AND INSTR(LOWER(name), ")
                    .bind(name)
                    .push_static(") > 0"),
                DbSqlFlavor::Postgres => statement
                    .push_static(" AND POSITION(")
                    .bind(name)
                    .push_static(" IN LOWER(name)) > 0"),
            };
        }
        if let Some(status) = query.status {
            statement = statement.push_static(" AND status = ").bind(match status {
                bcs_service_api::ActorStatus::Online => "online",
                bcs_service_api::ActorStatus::Hidden => "hidden",
            });
        }
        let statement = statement
            .push_static(" ORDER BY gmt_create DESC, bot_uuid ASC")
            .build();
        let rows = self
            .db_query(statement)
            .await
    ...
    async fn patch_control_plane(
        ...
        let bot_info_update = if let Some(descriptor) = patch.descriptor.as_ref() {
            let statement = DbStatementBuilder::new(self.flavor)
                .push_static("SELECT bot_info FROM bcs_bots WHERE bot_uuid = ")
                .bind(bot_id)
                .push_static(" AND env = ")
        ...
        let mut statement =
            DbStatementBuilder::new(self.flavor).push_static("UPDATE bcs_bots SET ");
        ...
        if let Some(status) = patch.status {
            ...
            statement = statement.push_static("status = ").bind(match status {
                bcs_service_api::ActorStatus::Online => "online",
                bcs_service_api::ActorStatus::Hidden => "hidden",
            });
            has_assignment = true;
        }
        if let Some(bot_info) = bot_info_update.as_deref() {
            if has_assignment {
                statement = statement.push_static(", ");
            }
            statement = statement.push_static("bot_info = ").bind(bot_info);
            has_assignment = true;
        }
        if has_assignment {
            statement = statement.push_static(", ");
        ...
        let statement = statement
            .push_static(
                "gmt_modified = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP \
                 WHERE bot_uuid = ",
            )
            .bind(bot_id)
            .push_static(" AND env = ")
...
mod tests {
    ...
    #[derive(Default)]
    struct RecordingDb {
        queries: StdMutex<Vec<DbStatement>>,
        executes: StdMutex<Vec<DbStatement>>,
    }
    ...
    impl DbPlugin for RecordingDb {
        ...
        async fn execute(&self, statement: DbStatement) -> DbResult<DbExecuteResult> {
            self.executes.lock().expect("executes").push(statement);
            Ok(DbExecuteResult {
                affected_rows: 1,
                last_insert_id: None,
            })
            ...
            Ok(steps
                ...
                .map(|(step_index, step)| match step {
                    DbTransactionStep::Query(statement) => {
                        self.queries.lock().expect("queries").push(statement);
                        Ok(DbTransactionStepResult::Rows(Vec::new()))
                    }
                    DbTransactionStep::Execute(statement) => {
                        self.executes.lock().expect("executes").push(statement);
                        Ok(DbTransactionStepResult::Executed(DbExecuteResult {
                            affected_rows: 1,
                            last_insert_id: None,
                        }))
                    }
    ...
    fn postgres_registry(db: Arc<RecordingDb>) -> PersistentBotRepo {
        PersistentBotRepo::with_plugins_flavor(
            Arc::new(bcs_cache_local::InMemoryCachePlugin::new()),
            db,
            DbSqlFlavor::Postgres,
        )
    }
    ...
    #[tokio::test]
    async fn postgres_bot_insert_uses_contiguous_typed_parameters() {
        let db = Arc::new(RecordingDb::default());
        let registry = postgres_registry(db.clone());
        let capabilities = BotCapabilities {
            name: Some("PostgreSQL Bot".to_string()),
        ...
        registry
            ...
            .await
            .expect("insert PostgreSQL bot");

        let executes = db.executes.lock().expect("executes");
        assert_eq!(executes.len(), 1);
        assert!(!executes[0].sql().contains('?'));
        assert!(executes[0].sql().contains("$11"));
        assert_eq!(executes[0].params().len(), 11);
    }

    #[tokio::test]
    async fn postgres_human_insert_has_exact_conflict_target() {
        let db = Arc::new(RecordingDb::default());
        let registry = postgres_registry(db.clone());

        registry
            .ensure_human_actor("staff-1", "Human One")
            .await
            .expect("ensure PostgreSQL Human actor");

        let executes = db.executes.lock().expect("executes");
        assert_eq!(executes.len(), 1);
        assert!(!executes[0].sql().contains('?'));
        assert!(executes[0].sql().contains("$9"));
        assert!(
            executes[0]
                .sql()
                .contains("ON CONFLICT(bot_uuid, env) DO NOTHING")
        );
    }

    #[tokio::test]
    async fn postgres_candidate_cte_and_paging_use_contiguous_parameters() {
        let db = Arc::new(RecordingDb::default());
        let registry = postgres_registry(db.clone());
        ...
        registry
            .list_control_plane_candidates(BotCandidateReadQuery {
                ...
                friend_ids: std::collections::HashSet::from([
                    "friend-b".to_string(),
                    "friend-a".to_string(),
                ]),
            ...
            .await
            .expect("list PostgreSQL bot candidates");

        let queries = db.queries.lock().expect("queries");
        assert_eq!(queries.len(), 2);
        assert!(!queries[0].sql().contains('?'));
        assert!(queries[0].sql().contains("SELECT $1 AS bot_uuid"));
        assert!(queries[0].sql().contains("UNION ALL SELECT $2"));
        assert!(queries[0].sql().contains("POSITION($6 IN LOWER(b.name))"));
        assert!(queries[0].sql().contains("LIMIT $7 OFFSET $8"));
        assert_eq!(queries[0].params().len(), 8);
        assert!(!queries[1].sql().contains('?'));
        assert!(queries[1].sql().contains("POSITION($6 IN LOWER(b.name))"));
        assert_eq!(queries[1].params().len(), 6);
    }
