...
pub async fn organization_repo_contract_tests<T: OrganizationRepoPort + ?Sized>(repo: &T) {
    ...
    assert!(
        repo.list_members(ListOrganizationMembersQuery {
            env: "contract".to_string(),
            organization_code: "promo-2026".to_string(),
...
pub async fn group_repo_contract_tests<T: GroupRepoPort + ?Sized>(repo: &T) {
    ...
    let stored = repo
        .get(&group.id)
        .await
        .expect("group after mutable patch");
    ...
    let spec = fetched.service_spec.expect("service_spec should roundtrip");
...
pub async fn session_repo_contract_tests<T: SessionRepoPort + ?Sized>(repo: &T) {
    ...
    // Not collected yet
    assert!(
        repo.list_collected_by_group(&svc.group_id, "bot-collector", None, None, 0, 10)
            .await
            .is_empty()
    );
    ...
    assert!(
        empty.is_empty(),
        "collected_at_map must be empty for non-collector"
    );

    // per-bot isolation: other participant sees nothing
    assert!(
        repo.list_collected_by_group(&svc.group_id, "bot-other", None, None, 0, 10)
            .await
            .is_empty()
    );
    ...
    let listed_all = repo
        .list_by_group(group_id, None, 0, 1000, None, None)
        .await;
    assert_eq!(
...
pub async fn message_repo_contract_tests<T: MessageRepoPort + ?Sized>(repo: &T) {
    ...
    // get_current_seq
    let seq = repo
        .get_current_seq(session_id)
        .await
        .expect("get_current_seq");
    assert_eq!(seq, 6);
    ...
    assert!(
        page4
            .messages
            .iter()
            .all(|m| m.message_type == "system_event")
    );
    ...
    assert_eq!(
        owner_page.messages[0].owner_bot_id.as_deref(),
        Some("workerA")
    );
    ...
    assert!(
        public_or_wa
            .messages
            .iter()
            .all(|m| { m.owner_bot_id.is_none() || m.owner_bot_id.as_deref() == Some("workerA") })
    );
    assert!(
