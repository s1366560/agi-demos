...
{\"ok\":",
    ...
    fn abort_provider_request_preserves_target_run_id() {
        ...
        let body = provider_request_from_frame(&target, &frame, 30_000).unwrap();

        assert_eq!(body.run_id.as_deref(), Some("target-run-1"));
        assert_eq!(body.session_id, "session-1");
        assert_eq!(
            serde_json::to_value(body).unwrap()["run_id"],
            "target-run-1"
        );
    }
